...
SET  @begin_time = '2021-11-16 00:00:00';
...
SELECT  DATE_FORMAT(ro.last_updated_date,'%Y-%m-%d %H:00:00') times,
    ...
    JOIN evo_wcs_g2p.guided_put_away_job wpj
      ON ro.id = wpj.order_id
	WHERE ro.state = 'DONE' AND ro.last_updated_date >= @begin_time and ro.last_updated_date < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num MINUTE) AND ro.project_code = 'A51118' AND wpj.project_code = 'A51118'
...
SELECT DATE_FORMAT(cwd.updated_date,'%Y-%m-%d %H:00:00') times,
     ...
     0 'sku_num',
     0 'into_station_times'
    FROM evo_wcs_g2p.guided_putaway_work_detail cwd
	JOIN evo_wcs_g2p.guided_put_away_job wpj
	  ON cwd.detail_id = wpj.detail_id
	WHERE cwd.quantity = cwd.fulfill_quantity AND wpj.state= 'DONE' AND cwd.updated_date >= @begin_time and cwd.updated_date < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num MINUTE) AND cwd.project_code = 'A51118' AND wpj.project_code = 'A51118'
...
SELECT DATE_FORMAT(j.updated_date,'%Y-%m-%d %H:00:00') times,
	...
	WHERE j.state='DONE' AND j.updated_date >= @begin_time and j.updated_date < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num MINUTE) AND j.project_code = 'A51118' 
