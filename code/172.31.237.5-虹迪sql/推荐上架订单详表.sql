SELECT  
     ...
     cast(SUM(tmp2.time) as decimal(10,2)) as '平均人工盘点耗时' -- '平均人工盘点耗时/秒'
FROM (
...
SELECT 
   ...
   0 'station_used',
    ...
    WHERE 
       ...
       AND sl.biz_type LIKE '%GUIDED%' AND sl.project_code = 'C35052' 
        ) tt1
...
WHERE sc.state = 'PULLED' AND sc.last_updated_date BETWEEN DATE_ADD(DATE_FORMAT(sysdate(),'%Y-%m-%d 00:00:00'),INTERVAL -1 DAY) AND DATE_FORMAT(sysdate(),'%Y-%m-%d 00:00:00') AND sc.project_code = 'A51118' AND t.project_code = 'A51118' AND pj.project_code = 'A51118' 
)tmp
ON jsc.job_id = tmp.job_id
WHERE jsc.updated_date >= DATE_ADD(DATE_FORMAT(sysdate(),'%Y-%m-%d 00:00:00'),INTERVAL -1 DAY) AND jsc.updated_date < DATE_FORMAT(sysdate(),'%Y-%m-%d 00:00:00') AND jsc.state = 'DONE' AND jsc.project_code = 'A51118'  AND pj.project_code = 'A51118' 
