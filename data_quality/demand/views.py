...
def list_subproductname(request):
    ...
    try:
        sql = f"""select rownum,
                    ...
                    where id in ( select max(id) from source_system_demand where productname='{productname}' group by productname,item_name,quarter )
                    group by rownum,productname"""
        curs.execute(sql)
