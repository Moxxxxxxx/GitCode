...
from monitor.utils.str import gen_md5
...
class MyRDBMS:
    ...
    def execute(self, *args, **kwargs):
        ...
        with self.db_session() as db:
            # 查询数据是否存在
            data: CursorResult = db.execute(*args, **kwargs)
            db.commit()
            return data
    ...
    def __save_data(self, table_name: str, pk: (tuple, list), data_rows: (tuple, list)):
        ...
        for __line_data in data_rows:
            if not isinstance(__line_data, (dict,)):
                continue

            # 如果值是不存在，那么就替换成 None
            for k, v in __line_data.items():
                if not v or v in ["", " "]:
                    __line_data[k] = None
