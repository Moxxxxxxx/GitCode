...
with DAG(
    ...
    # nonetwork = SSHOperator(
    #     ssh_conn_id='qkt_ssh008',
    #     task_id='nonetwork',
    #     command='python3 /opt/docker/offline-script/nonetwork.py /opt/docker/offline/ /opt/docker/offline-txt/ /opt/docker/offline-bak/',
    #     dag=dag
    # )
    ...
    # 创建live-ods表数据源
    live_ods_list = readerFileList(None, 'live2_ods_task.txt')

    live_ods_task = f'''sh {live2_quick_airflow_ods}task.sh {yesterday} '''

    live_ods_task_scheduling = []

    live_ods_task_map = {}

    shareTaskOperator(live_ods_list, live_ods_task, live_ods_task_scheduling, live_ods_task_map)

    root  >> live_ods_task_scheduling >> end
