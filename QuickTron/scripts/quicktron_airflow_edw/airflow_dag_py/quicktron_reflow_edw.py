...
with DAG(
        dag_id='quicktron_reflow_edw',
        schedule_interval='50 03 * * *',
        start_date=pendulum.datetime(2022, 11, 22, tz="Asia/Shanghai"),
        default_args=default_args,
        catchup=False,  # 执行DAG时，将开始时间到目前所有该执行的任务都执行，默认为True
        concurrency=5,  # 设置DAG的最大运行 task任务数
    ...
    def readerFileList(yesterday, *file_paths):
        ...
        *file_paths  需要读取的文件名
        ...
        for file_path in file_paths:
            ...
            if file_name.startswith('live2_ads'):
                hive_sh = '/live2/ads/'
        ...
        for item in file_strs:
            if item.startswith("reflow_"):
                ...
                for x in value.split("&"):
                    share_depend_set.add(x.strip())
        file_strs.close()
        return share_depend_map, share_depend_set
    ...
    def shareExternalTask(set_1):

        list_task_scheduling = list()

        map_1 = dict()

        for share_dag in set_1:
            share_task = ExternalTaskSensor(
                task_id=share_dag,
                external_dag_id="quicktron_main_edw",  # 需要等待的外部DAG id
                external_task_id=share_dag,  # 需要等待的外部Task id
                execution_delta=datetime.timedelta(minutes=50),
                ...
                timeout=14400,  # 超时时间，如果等待了600秒还未符合期望状态的外部Task，那么抛出异常进入重试
        ...
        return list_task_scheduling, map_1
    ...
    ####
    share_map_list_task = readerFileList(None, 'dic_ads_task.txt', 'live2_ads_task.txt')  # 获取需要执行的脚本以及脚本所在的路径

    ####
    share_depend_map, share_depend_set = readerFlieMap('dic_task_schedule.txt')  # 获取task任务之间依赖关系map字典

    ####
    trigger_task_scheduling, trigger_task_map = shareExternalTask(share_depend_set)  # 获取外部dag的外部算子
