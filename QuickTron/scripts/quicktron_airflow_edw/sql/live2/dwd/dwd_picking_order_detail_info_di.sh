...
#-- 运行类型 ： 日跑
#-- 参数 ：     d 
#-- 功能描述 ： 项目 dwd层  出库作业单明细
#-- 注意 ： 每日按天增量分区
#-- 输入表 : ods.ods_qkt_picking_order_detail_di
#-- 输出表 ：dwd.dwd_picking_order_detail_info_di
...
with tmp_order_detail_str1 as (
select 
distinct substr(created_date,0,10) as d,project_code
from
${ods_dbname}.ods_qkt_picking_order_detail_di
where d='${pre1_date}' 
and substr(created_date,0,10)<>'${pre1_date}'
),
tmp_order_detail_str2 as (
select 
b.*
from 
tmp_order_detail_str1 a
inner join ${dwd_dbname}.dwd_picking_order_detail_info_di b on a.d=b.d and a.project_code=b.pt
)
insert overwrite table ${dwd_dbname}.dwd_picking_order_detail_info_di partition(d,pt)
select 
id,
external_id,
picking_order_id,
