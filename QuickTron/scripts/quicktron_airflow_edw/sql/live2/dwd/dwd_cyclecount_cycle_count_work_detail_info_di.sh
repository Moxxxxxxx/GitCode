...
#-- 运行类型 ： 日跑
#-- 参数 ：     d 
#-- 功能描述 ： 项目 dwd层  盘点作业单明细
#-- 注意 ： 每日按天增量分区
#-- 输入表 : ods.ods_qkt_cyclecount_cycle_count_work_detail_di
#-- 输出表 ：dwd.dwd_cyclecount_cycle_count_work_detail_info_di
...
insert overwrite table ${dwd_dbname}.dwd_cyclecount_cycle_count_work_detail_info_di partition(d,pt)
select 
id,
warehouse_id,
cycle_count_work_id,
...
from
${ods_dbname}.ods_qkt_cyclecount_cycle_count_work_detail_di
) t
where t.rn=1
...
with tmp_cycle_count_work_detail_str1 as (
select 
...
tmp_cycle_count_work_detail_str2 as (
select 
b.*
from 
tmp_cycle_count_work_detail_str1 a
inner join ${dwd_dbname}.dwd_cyclecount_cycle_count_work_detail_info_di b on a.d=b.d and a.project_code=b.pt
)
insert overwrite table ${dwd_dbname}.dwd_cyclecount_cycle_count_work_detail_info_di partition(d,pt)
select 
id,
...
d
from 
tmp_cycle_count_work_detail_str2
) t
) rt 
where rt.rn=1
;
