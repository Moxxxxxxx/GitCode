...
#-- 功能描述 ： 项目 dwd层  盘点作业单
#-- 注意 ： 每日按天增量分区
#-- 输入表 : ods.ods_qkt_cyclecount_cycle_count_work_di
#-- 输出表 ：dwd.dwd_cyclecount_cycle_count_work_info_di
...
with tmp_cycle_count_work_str1 as (
select 
distinct substr(created_date,0,10) as d,project_code
from
${ods_dbname}.ods_qkt_cyclecount_cycle_count_work_di
where d='${pre1_date}' 
and substr(created_date,0,10)<>'${pre1_date}'
),
tmp_cycle_count_work_str2 as (
select 
b.*
from 
tmp_cycle_count_work_str1 a
inner join ${dwd_dbname}.dwd_cyclecount_cycle_count_work_info_di b on a.d=b.d and a.project_code=b.pt
)
insert overwrite table ${dwd_dbname}.dwd_cyclecount_cycle_count_work_info_di partition(d,pt)
select 
id,
warehouse_id,
cycle_count_id,
