...
(
  SELECT *
  FROM ${dim_dbname}.dim_collection_project_record_ful
  WHERE project_product_type IN ('Quickpick','料箱搬运QP','QP') OR project_code IN ('A51274','C35052')
)t2
JOIN 
(
-- 搬运作业单时间链路
...
$hive -e "$sql" && hive_concatenate ads ads_carry_work_analyse_detail ${pre1_date}