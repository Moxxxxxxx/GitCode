...
#-- 功能描述 ： 采集ones 任务状态表
#-- 注意 ： 每天全量，每天一个快照
#-- 输入表 : quality_data.task_status
#-- 输出表 ：ods.ods_qkt_ones_task_status_df
...
######### 设置表的变量
dbname=ods
table=ods_qkt_ones_task_status_df
outdir=/data/sqoop/logs/hcatalog
target_dir=/warehouse/hive/ods/
mysql_dbname=quality_data
mysql_table=task_status
...
--query "select
uuid, team_uuid, name, name_pinyin, category, built_in, create_time, status, detail_type
...
insert overwrite table ods.ods_qkt_ones_task_status_df partition(d='${pre1_date}')
select 
uuid, 
team_uuid, 
...
detail_type
from 
ods.ods_qkt_ones_task_status_df a
