...
#-- 1 wangyingying 2022-05-30 创建
#-- 2 wangyingying 2022-11-30 增加数据来源和项目信息字段
#-- 3 wangyingying 2023-02-13 增加是否活跃字段
#-- 4 wangyingying 2023-02-14 优化PE日志逻辑
# ------------------------------------------------------------------------------------------------
...
(
  SELECT days -- 日期
  FROM ${dim_dbname}.dim_day_date -- 日期维表
  WHERE is_month_begin = 1 AND days >= '2021-07-01' AND days <= '${pre1_date}' -- 只取2021年7月1日之后的日期补零
  GROUP BY days
)d
LEFT JOIN 
(
...
(
  SELECT project_code,
         ...
         project_area_group,
		 is_active,
         TO_DATE(CONCAT(SUBSTR(days,1,7),'-','01')) AS cur_month,
         emp_id
  FROM pe_log
  WHERE project_code is not null AND day_type != '全天请假' AND ischeck = '已打卡' AND is_valid = 1 -- 只取有效数据
)l
