...
#-- ods.ods_qkt_rcs_basic_agv_df、ods.ods_qkt_rcs_basic_agv_type_df、ods.ods_qkt_rcs_basic_charger_df、ods.ods_qkt_station_station_df
...
#-- 18 wangziming 2022-05-05 modify 增加项目结项时间 post_project_date
#-- 19 wangziming 2022-05-11 modify 修改项目的优先级的清洗规则逻辑
#-- 20 wangziming 2022-05-39 modify 修改 up_spm的取值逻辑，按照 pmid和spmid进行分组取值
#-- 21 wangziming 2022-06-10 modify 修改项目等级的规则，加上 `级`
#-- 22 wangziming 2022-08-02 modify 增加 agv_num，charger_num，station_num，agv_type_code，agv_type_name，agv_type_num
#-- 23 wangziming 2022-08-04 modify 增加 pe_members 现场pe成员，取昨天的pe日志
# ------------------------------------------------------------------------------------------------
...
select 
*,
row_number() over(partition by pmid,spmid order by id desc) rn
from 
...
select 
project_code,
concat_ws(',',collect_list(if(agv_type_code is not null,agv_type_code,null))) as agv_type_code,
concat_ws(',',collect_list(if(agv_type_name is not null,agv_type_name,null))) as agv_type_name,
concat_ws(',',collect_list(if(agv_type_name is not null,cast(num as string),null))) as agv_type_num,
sum(if(agv_type_code is not null,num,0)) as agv_num
from 
(
select 
a.project_code,
...
left join ${ods_dbname}.ods_qkt_rcs_basic_agv_type_df b on a.project_code=b.project_code 
and a.agv_type_id=b.id and b.d='${pre1_date}'
where a.d='${pre1_date}'
group by a.project_code,
a.agv_code,
b.agv_type_code,
b.agv_type_name
grouping sets(a.project_code,(a.project_code,b.agv_type_code,b.agv_type_name))
) t
group by project_code
) rt 
left join (
select 
project_code,
count(1) as charger_num
from 
${ods_dbname}.ods_qkt_rcs_basic_charger_df 
where d='${pre1_date}'
group by project_code
) rt1 on rt.project_code=rt1.project_code
left join (
select 
project_code,
count(1) as station_num
from 
${ods_dbname}.ods_qkt_basic_station_df 
where d='${pre1_date}'
group by project_code
) rt2 on rt.project_code=rt2.project_code
),
tmp_pe_members_str1 as ( -- 项目pe成员
select 
project_code,
concat_ws(',',collect_set(emp_name)) as pe_members
from 
(
select 
...
left join ${ods_dbname}.ods_qkt_dtk_user_info_df b on a.originator_userid =b.user_id and b.d='${pre1_date}'
where a.d='${pre1_date}' and substr(a.start_work_time,1,10)='${pre1_date}' 
) t
where nvl(project_code,'')<>''
group by project_code
)
...
concat((case when t1.project_priority rlike '^[0-9]{1}$' then  t1.project_priority
	 else '0' end),'级') as project_priority,
t11.sap_counselor,
t12.sap_entry_date,
...
if(project_code rlike '^FH-','0.前置',regexp_replace(project_operation_state,'\\\\s+','')) as project_operation_state,
project_context,
project_area,
...
regexp_replace(a.project_dispaly_state,'\\\\s+','') as project_dispaly_state
from 
bpm_project_base a
...
select 
project_code,
max(post_project_date) as post_project_date
from 
(
select 
upper(a.string12) as project_code,
substr(a.date5,1,10) as  post_project_date
from 
${ods_dbname}.ods_qkt_bpm_app_k3flow_df a
left join 
(
...
where a.d='${pre1_date}' 
and a.oFlowModelID='81695' 
and nvl(a.string12,'')<>''
and b.status='30'
) t
group by project_code 
) t15 on t1.project_code=t15.project_code
left join tmp_project_base_num_str1 t16 on t1.project_code=t16.project_code
left join tmp_pe_members_str1 t17 on t1.project_code=t17.project_code
;
"
