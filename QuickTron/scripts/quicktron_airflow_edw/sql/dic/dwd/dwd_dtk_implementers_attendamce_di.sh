...
#-- 4 wangziming 2022-06-15 modify 进行签到或签退时间为null的字段过滤
#-- 5 wangziming 2022-09-14 modify 重新进行初始化
#-- 6 wangziming 2022-11-30 modify service_type进行置空处理
# ------------------------------------------------------------------------------------------------
...
approval_title, 
project_code, 
if(nvl(service_type,'')='',null,service_type) as service_type, 
...
where rt.rn=1 and nvl(checkin_time,'')<>'' and nvl(checkout_time,'')<>''
;
"
