...
#-- 4 wangziming 2022-09-13 modify 修改签到数量到逻辑（where a.checkin_date=substr(a.checkin_times[0],1,10)），并重新初始化
#-- 5 wangziming 2022-09-24 modify 修改 跨天签到，前一天无签到信息记录逻辑
#-- 6 wangziming 2022-09-27 modify 重新初始化
#-- 7 wangziming 2022-10-11 modify 重新初始化
# ------------------------------------------------------------------------------------------------
...
tmp_dtk_group_day_checkin_str4 as (
select 
a.*
from 
tmp_dtk_group_day_checkin_str3 a
left join tmp_dtk_group_day_checkin_str2 b on a.emp_id=b.emp_id and a.checkin_date=b.checkin_date
...
where a.checkin_date=substr(a.checkin_times[0],1,10)
...
a.checkin_date as d
from 
tmp_dtk_group_day_checkin_str4 a 
left join ${dwd_dbname}.dwd_dtk_emp_info_df b on a.emp_id=b.emp_id and b.d='${pre1_date}'
;
"
