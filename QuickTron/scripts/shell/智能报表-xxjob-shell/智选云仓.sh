...
SELECT ceil(TIMESTAMPDIFF(SECOND, @begin_time,stg.updated_date)/@interval_time/60) times,
	...
	WHERE pj.state='DONE' AND stg.updated_date >= @begin_time and stg.updated_date < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num MINUTE) AND pj.project_code = 'A51246' AND stg.project_code = 'A51246'
...
SELECT  ceil(TIMESTAMPDIFF(SECOND, @begin_time, pw.updated_date)/@interval_time/60) times,
	...
	WHERE pw.state = 'DONE' AND pw.updated_date >= @begin_time and pw.updated_date < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num  MINUTE) AND pw.project_code = 'A51246'
...
SELECT ceil(TIMESTAMPDIFF(SECOND, @begin_time,pwd.updated_date)/@interval_time/60) times,
	...
	WHERE pwd.quantity = pwd.fulfill_quantity AND pj.state= 'DONE'  AND pwd.updated_date >= @begin_time and pwd.updated_date < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num MINUTE) AND pwd.project_code = 'A51246' AND pj.project_code = 'A51246'
...
SELECT ceil(TIMESTAMPDIFF(SECOND, @begin_time,se.entry_time)/@interval_time/60) times,
	...
	WHERE idempotent_id LIKE '%G2PPicking%' and se.entry_time >= @begin_time AND se.entry_time < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num MINUTE) AND se.project_code = 'A51246'
...
SELECT ceil(TIMESTAMPDIFF(SECOND, @begin_time,se.entry_time)/@interval_time/60) times,
     ...
     0 'time'
    FROM evo_station.station_entry se
	WHERE se.idempotent_id LIKE '%G2PPicking%' and se.entry_time >= @begin_time AND se.entry_time < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num MINUTE) AND se.project_code = 'A51246'
...
SELECT 
   ...
   SUM(		
    ...
    WHERE 
       ...
       AND seq.idempotent_id LIKE '%G2PPicking%' AND seq.project_code = 'A51246'
    ) tmp1
...
SELECT 
   ...
   0 'station_used',
    ...
    WHERE 
       ...
       AND sl.biz_type = 'PICKING_ONLINE_G2P_B2P' AND sl.project_code = 'A51246'
        ) tt1
...
WHERE sc.state = 'PULLED' AND sc.last_updated_date BETWEEN @begin_time AND DATE_ADD(@begin_time,INTERVAL 1 DAY) AND sc.project_code = 'A51246'  AND t.project_code = 'A51246' AND g.project_code = 'A51246' AND pj.project_code = 'A51246'
)tmp
ON jsc.job_id = tmp.job_id
WHERE jsc.updated_date >= @begin_time AND jsc.updated_date < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num MINUTE) AND jsc.state = 'DONE' AND jsc.project_code = 'A51246'  AND pj.project_code = 'A51246'
...
# 删除重复数据
mysql -h${HOSTNAME}  -P${PORT}  -u${USERNAME} -p${PASSWORD} ${DBNAME} -e "delete from smart_report.eff_index where date(created_date) = date(sysdate()) AND project_code = 'A51246';"
mysql -h${HOSTNAME}  -P${PORT}  -u${USERNAME} -p${PASSWORD} ${DBNAME} -e "delete from smart_report.eff_index_time where date(created_date) = date(sysdate()) AND project_code = 'A51246';"
mysql -h${HOSTNAME}  -P${PORT}  -u${USERNAME} -p${PASSWORD} ${DBNAME} -e "delete from smart_report.station_free where date(created_date) = date(sysdate()) AND project_code = 'A51246';"
echo "delete ok"
