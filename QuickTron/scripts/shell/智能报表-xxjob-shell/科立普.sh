...
SELECT  ceil(TIMESTAMPDIFF(SECOND, @begin_time, pw.updated_date)/@interval_time/60) times,
	...
	WHERE pw.state = 'DONE' AND pw.updated_date >= @begin_time and pw.updated_date < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num  MINUTE) AND pw.project_code = 'A51118'
...
INSERT INTO smart_report.station_free(station_code,\`07:00\`,\`08:00\`,\`09:00\`,\`10:00\`,\`11:00\`,\`12:00\`,\`13:00\`,\`14:00\`,\`15:00\`,\`16:00\`,\`17:00\`,\`18:00\`,\`19:00\`,\`20:00\`,\`21:00\`,\`22:00\`,\`23:00\`,\`00:00\`,\`01:00\`,\`02:00\`,\`03:00\`,\`04:00\`,\`05:00\`,\`06:00\`,project_code)
SELECT 
...
# 删除重复数据
mysql -h${HOSTNAME}  -P${PORT}  -u${USERNAME} -p${PASSWORD} ${DBNAME} -e "delete from smart_report.eff_index where date(created_date) = date(sysdate()) AND project_code = 'A51118';"
mysql -h${HOSTNAME}  -P${PORT}  -u${USERNAME} -p${PASSWORD} ${DBNAME} -e "delete from smart_report.eff_index_time where date(created_date) = date(sysdate()) AND project_code = 'A51118';"
mysql -h${HOSTNAME}  -P${PORT}  -u${USERNAME} -p${PASSWORD} ${DBNAME} -e "delete from smart_report.station_free where date(created_date) = date(sysdate()) AND project_code = 'A51118';"
echo "delete ok"
