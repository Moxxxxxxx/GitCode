...
######### 设置表的变量
dbname=ods
table=ods_qkt_station_station_task
outdir=/data/sqoop/logs/hcatalog
target_dir=/warehouse/hive/ods/
mysql_dbname=evo_station
mysql_table=station_task
...
###################### station_task->ods_qkt_station_station_task(T-1每日增量->分区)
...
## 参数
columns=id,task_no,warehouse_id,task_type,biz_type,task_level,hierarchy,parent_task_id,station_id,station_code,station_point_code,agv_code,bucket_code,bucket_face,work_id,state,channel,created_date,created_user,created_app,last_updated_date,last_updated_user,last_updated_app,project_code
### 第一次初始化脚本
init_sql="
...
use $dbname;
insert overwrite table dwd.dwd_station_station_task_info partition(d,pt)
select 
id,
task_no,
warehouse_id,
...
*
,row_number() over(partition by id,project_code order by last_updated_date desc) as rn
from ods_qkt_station_station_task
