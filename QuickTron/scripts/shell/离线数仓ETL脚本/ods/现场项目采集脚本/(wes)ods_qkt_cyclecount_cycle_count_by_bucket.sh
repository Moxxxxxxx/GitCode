...
######### 设置表的变量
dbname=ods
table=ods_qkt_cyclecount_cycle_count_by_bucket
outdir=/data/sqoop/logs/hcatalog
target_dir=/warehouse/hive/ods/
mysql_dbname=evo_wes_cyclecount
mysql_table=cycle_count_by_bucket
...
###################### cycle_count_by_bucket->ods_qkt_cyclecount_cycle_count_by_bucket(T-1每日增量->分区)
...
## 参数
columns=id,warehouse_id,cycle_count_id,bucket_code,bucket_face,bucket_slot_code,delete_flag,created_date,created_user,created_app,last_updated_date,last_updated_user,last_updated_app,project_code
### 第一次初始化脚本
init_sql="
...
use $dbname;
insert overwrite table dwd.dwd_cyclecount_cycle_count_by_bucket_info partition(d,pt)
select 
id,
warehouse_id,
cycle_count_id,
...
from
ods_qkt_cyclecount_cycle_count_by_bucket 
) t
where t.rn=1
...
*
,row_number() over(partition by id,project_code order by last_updated_date desc) as rn
from ods_qkt_cyclecount_cycle_count_by_bucket
