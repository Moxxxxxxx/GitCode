...
######### 设置表的变量
dbname=ods
table=ods_qkt_g2p_order_group
outdir=/data/sqoop/logs/hcatalog
target_dir=/warehouse/hive/ods/
mysql_dbname=evo_wcs_g2p
mysql_table=order_group
...
###################### order_group->ods_qkt_g2p_order_group(T-1每日增量->分区)
...
--query "select id,warehouse_id ,order_group_id,order_type,station_code,take_effect,state,priority,priority_type,
created_app,created_date,updated_app,updated_date,order_group_type,project_code from $mysql_table  
...
## 参数
columns=id,warehouse_id,order_group_id,order_type,station_code,take_effect,state,priority,priority_type,created_app,created_date,updated_app,updated_date,order_group_type,project_code
### 第一次初始化脚本
init_sql="
...
use $dbname;
insert overwrite table dwd.dwd_g2p_order_group_info partition(d,pt)
select 
id,
warehouse_id,
order_group_id,
