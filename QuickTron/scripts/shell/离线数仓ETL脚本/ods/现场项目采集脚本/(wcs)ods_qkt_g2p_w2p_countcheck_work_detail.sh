...
######### 设置表的变量
dbname=ods
table=ods_qkt_g2p_w2p_countcheck_work_detail
outdir=/data/sqoop/logs/hcatalog
target_dir=/warehouse/hive/ods/
mysql_dbname=evo_wcs_g2p
mysql_table=w2p_countcheck_work_detail_v2
...
###################### w2p_countcheck_work_detail_v2->ods_qkt_g2p_w2p_countcheck_work_detail(T-1每日增量->分区)
...
updated_app,
updated_date,
project_code from $mysql_table  where date_format($datax_incre_column,'%Y-%m-%d')=date_add('$pre1_date',interval 1 day) and \$CONDITIONS"  \
--num-mappers 2 \
--split-by id \
...
## 参数
columns=id,warehouse_id,zone_code,work_id,source_detail_id,level1_container_code,state,created_app,created_date,updated_app,updated_date,project_code
### 第一次初始化脚本
init_sql="
