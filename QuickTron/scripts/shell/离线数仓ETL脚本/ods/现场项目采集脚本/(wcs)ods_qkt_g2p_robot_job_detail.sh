...
######### 设置表的变量
dbname=ods
table=ods_qkt_g2p_robot_job_detail
outdir=/data/sqoop/logs/hcatalog
target_dir=/warehouse/hive/ods/
mysql_dbname=evo_wcs_g2p
mysql_table=robot_job_detail
...
###################### robot_job_detail->ods_qkt_g2p_robot_job_detail(T-1每日增量量->分区)
...
## 参数
columns=id,warehouse_id,job_id,robot_job_id,job_type,state,des,reason,agv_code,created_app,created_date,updated_app,updated_date,project_code
### 第一次初始化脚本
init_sql="
...
use $dbname;
insert overwrite table dwd.dwd_g2p_robot_job_detail_info partition(d,pt)
select 
id,
warehouse_id,
job_id,
...
*
,row_number() over(partition by id,project_code order by updated_date desc) as rn
from ods_qkt_g2p_robot_job_detail
