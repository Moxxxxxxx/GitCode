...
######### 设置表的变量
dbname=ods
table=ods_qkt_picking_order_fulfill_detail
outdir=/data/sqoop/logs/hcatalog
target_dir=/warehouse/hive/ods/
mysql_dbname=evo_wes_picking
mysql_table=picking_order_fulfill_detail
...
###################### picking_order_fulfill_detail->ods_qkt_picking_order_fulfill_detail(T-1每日增量量->分区)
...
## 参数
columns=id,picking_order_detail_id,tenant_id,state,sku_id,owner_code,pack_id,lot_id,station_slot_id,station_slot_code,station_code,quantity,short_pick,container_code,package_uuid,level3_inventory_id,bucket_code,bucket_slot_code,job_id,short_pick_flag,short_pick_reason,location_container_code,version,operator,warehouse_id,delete_flag,created_date,created_user,created_app,last_updated_date,last_updated_user,last_updated_app,frozen_flag,project_code
### 第一次初始化脚本
init_sql="
...
use $dbname;
insert overwrite table dwd.dwd_picking_order_fulfill_detail_info partition(d,pt)
select 
id,
picking_order_detail_id,
tenant_id,
...
from
ods_qkt_picking_order_fulfill_detail 
) t
where t.rn=1
;
"
...
*
,row_number() over(partition by id,project_code order by last_updated_date desc) as rn
from ods_qkt_picking_order_fulfill_detail
