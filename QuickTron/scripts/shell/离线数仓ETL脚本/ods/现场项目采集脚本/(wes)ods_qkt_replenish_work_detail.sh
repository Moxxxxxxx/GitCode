...
######### 设置表的变量
dbname=ods
table=ods_qkt_replenish_work_detail
...
###################### replenish_work_detail->ods_qkt_replenish_work_detail(T-1每日增量量->分区)
...
## 参数
columns=id,replenish_work_id,tenant_id,owner_code,state,sku_id,lot_id,pack_id,unit_id,level1_container_code,level2_container_code,level3_container_code,station_slot_id,quantity,fulfill_quantity,use_frozen_flag,source_order_id,source_order_detail_id,version,zone_id,zone_code,warehouse_id,delete_flag,created_date,created_user,created_app,last_updated_date,last_updated_user,last_updated_app,project_code
### 第一次初始化脚本
init_sql="
...
use $dbname;
insert overwrite table dwd.dwd_replenish_work_detail_info partition(d,pt)
select 
id,
replenish_work_id,
tenant_id,
...
*
,row_number() over(partition by id,project_code order by last_updated_date desc) as rn
from ods_qkt_replenish_work_detail
