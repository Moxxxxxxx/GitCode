...
######### 设置表的变量
dbname=ods
table=ods_qkt_cyclecount_cycle_count_work
outdir=/data/sqoop/logs/hcatalog
target_dir=/warehouse/hive/ods/
mysql_dbname=evo_wes_cyclecount
mysql_table=cycle_count_work
...
###################### cycle_count_work->ods_qkt_cyclecount_cycle_count_work(T-1每日增量->分区)
...
## 参数
columns=id,warehouse_id,cycle_count_id,zone_code,zone_mode,owner_code,delete_flag,cycle_count_type,state,version,remark,operating_mode,udf1,udf2,udf3,udf4,udf5,created_date,created_user,created_app,last_updated_date,last_updated_user,last_updated_app,project_code
### 第一次初始化脚本
init_sql="
...
use $dbname;
insert overwrite table dwd.dwd_cyclecount_cycle_count_work_info partition(d,pt)
select 
id,
warehouse_id,
cycle_count_id,
...
from
ods_qkt_cyclecount_cycle_count_work 
) t
where t.rn=1
...
*
,row_number() over(partition by id,project_code order by last_updated_date desc) as rn
from ods_qkt_cyclecount_cycle_count_work
