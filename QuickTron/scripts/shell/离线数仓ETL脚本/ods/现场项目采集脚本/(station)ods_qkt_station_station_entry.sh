...
######### 设置表的变量
dbname=ods
table=ods_qkt_station_station_entry
outdir=/data/sqoop/logs/hcatalog
target_dir=/warehouse/hive/ods/
mysql_dbname=evo_station
mysql_table=station_entry
...
###################### station_entry->ods_qkt_station_station_entry(T-1每日增量->分区)
...
## 参数
columns=id,warehouse_id,station_code,biz_type,station_point_code,agv_code,bucket_code,entry_time,exit_time,idempotent_id,remark,delete_flag,created_date,created_user,created_app,last_updated_date,last_updated_user,last_updated_app,project_code
...
use $dbname;
insert overwrite table dwd.dwd_station_station_entry_info partition(d,pt)
select 
id,
warehouse_id,
station_code,
...
*
,row_number() over(partition by id,project_code order by last_updated_date desc) as rn
from ods_qkt_station_station_entry
