...
######### 设置表的变量
dbname=ods
table=ods_qkt_replenish_order_fulfill_detail
outdir=/data/sqoop/logs/hcatalog
target_dir=/warehouse/hive/ods/
mysql_dbname=evo_wes_replenish
mysql_table=replenish_order_fulfill_detail
...
###################### wes_replenish_order_fulfill_detail->ods_qkt_replenish_order_fulfill_detail(T-1每日增量量->分区)
...
use $dbname;
insert overwrite table dwd.dwd_replenish_order_fulfill_detail_info partition(d,pt)
select 
id,
replenish_order_id,
replenish_order_detail_id,
...
from
ods_qkt_replenish_order_fulfill_detail 
) t
where t.rn=1
;
"
...
*
,row_number() over(partition by id,project_code order by last_updated_date desc) as rn
from ods_qkt_replenish_order_fulfill_detail
