...
######### 设置表的变量
dbname=ods
table=ods_qkt_rcs_agv_job_event_notification
outdir=/data/sqoop/logs/hcatalog
target_dir=/warehouse/hive/ods/
mysql_dbname=evo_rcs
mysql_table=agv_job_event_notification
...
###################### agv_job_event_notification->ods_qkt_rcs_agv_job_event_notification(T-1每日增量->分区)
...
## 参数
columns=id,create_time,update_time,agv_code,event_content,event_name,is_sent_success,job_id,job_type,message_id,project_code
...
use $dbname;
insert overwrite table dwd.dwd_rcs_agv_job_event_notification_info partition(d,pt)
select 
id,
...
from
ods_qkt_rcs_agv_job_event_notification 
) t
where t.rn=1
;
"
...
*
,row_number() over(partition by id,project_code order by update_time desc) as rn
from ods_qkt_rcs_agv_job_event_notification
