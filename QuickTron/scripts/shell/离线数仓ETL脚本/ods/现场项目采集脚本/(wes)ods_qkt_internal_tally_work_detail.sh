...
######### 设置表的变量
dbname=ods
table=ods_qkt_internal_tally_work_detail
outdir=/data/sqoop/logs/hcatalog
target_dir=/warehouse/hive/ods/
mysql_dbname=evo_wes_internal
mysql_table=tally_work_detail
...
###################### tally_work_detail->ods_qkt_internal_tally_work_detail(T-1每日增量->分区)
...
## 参数
columns=id,tally_order_detail_id,tally_work_id,work_detail_type,state,dispersion_type,sku_id,lot_id,pack_id,owner_code,station_slot_id,station_slot_code,plan_total_quantity,real_total_quantity,warehouse_id,created_date,created_user,created_app,last_updated_date,last_updated_user,last_updated_app,project_code
### 第一次初始化脚本
init_sql="
...
use $dbname;
insert overwrite table dwd.dwd_internal_tally_work_detail_info partition(d,pt)
select 
id,
tally_order_detail_id,
tally_work_id,
...
from
ods_qkt_internal_tally_work_detail 
) t
where t.rn=1
;
"
...
*
,row_number() over(partition by id,project_code order by last_updated_date desc) as rn
from ods_qkt_internal_tally_work_detail
