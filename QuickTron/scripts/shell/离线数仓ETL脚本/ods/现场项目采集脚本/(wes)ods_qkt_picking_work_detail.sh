...
######### 设置表的变量
dbname=ods
table=ods_qkt_picking_work_detail
outdir=/data/sqoop/logs/hcatalog
target_dir=/warehouse/hive/ods/
mysql_dbname=evo_wes_picking
mysql_table=picking_work_detail
...
###################### picking_work_detail->ods_qkt_picking_work_detail(T-1每日增量量->分区)
...
## 参数
columns=id,picking_work_id,picking_order_detail_id,tenant_id,level2_inventory_id,level3_inventory_id,owner_code,sku_id,lot_id,use_frozen_flag,pack_id,state,quantity,fulfill_quantity,short_pick,version,zone_code,warehouse_id,delete_flag,created_date,created_user,created_app,last_updated_date,last_updated_user,last_updated_app,project_code
### 第一次初始化脚本
init_sql="
...
use $dbname;
insert overwrite table dwd.dwd_picking_work_detail_info partition(d,pt)
select 
id,
picking_work_id,
picking_order_detail_id,
...
*
,row_number() over(partition by id,project_code order by last_updated_date desc) as rn
from ods_qkt_picking_work_detail
