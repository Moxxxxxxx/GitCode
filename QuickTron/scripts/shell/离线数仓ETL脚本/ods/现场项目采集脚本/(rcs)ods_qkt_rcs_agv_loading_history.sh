...
######### 设置表的变量
dbname=ods
table=ods_qkt_rcs_agv_loading_history
outdir=/data/sqoop/logs/hcatalog
target_dir=/warehouse/hive/ods/
mysql_dbname=evo_rcs
mysql_table=agv_loading_history
...
###################### agv_loading_history->ods_qkt_rcs_agv_loading_history(T-1每日增量量->分区)
...
## 参数
columns=id,create_time,agv_code,is_virtual,position,top_face,type,value,cause,project_code,
...
use $dbname;
insert overwrite table dwd.dwd_rcs_agv_loading_history_info partition(d,pt)
select 
id,
...
from
ods_qkt_rcs_agv_loading_history 
) t
where t.rn=1
;
"
...
*
,row_number() over(partition by id,project_code order by create_time desc) as rn
from ods_qkt_rcs_agv_loading_history
