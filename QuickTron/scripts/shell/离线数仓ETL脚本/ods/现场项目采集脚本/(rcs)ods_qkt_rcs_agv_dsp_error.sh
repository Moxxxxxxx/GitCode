...
######### 设置表的变量
dbname=ods
table=ods_qkt_rcs_agv_dsp_error
outdir=/data/sqoop/logs/hcatalog
target_dir=/warehouse/hive/ods/
mysql_dbname=evo_rcs
mysql_table=rcs_agv_dsp_error
...
###################### rcs_agv_dsp_error->ods_qkt_rcs_agv_dsp_error(T-1每日全量->分区)
...
## 参数
columns=id,create_time,update_time,agv_code,bind_container,ctx_agv_state,ctx_job_content,ctx_job_id,ctx_job_name,db_agv_state,db_job_content,db_job_id,db_job_name,db_job_state,error,pd_info,recovery_agv_state,recovery_pd_info,project_code
### 第一次初始化脚本
init_sql="
...
#$hive $hive_username $hive_passwd -e "$init_sql"
...
use $dbname;
insert overwrite table dwd.dwd_rcs_agv_dsp_error_info partition(d,pt)
select 
id,
create_time as error_create_time,
update_time as error_update_time,
agv_code,
bind_container,
ctx_agv_state,
ctx_job_content,
...
*
,row_number() over(partition by id,project_code order by update_time desc) as rn
from ods_qkt_rcs_agv_dsp_error
