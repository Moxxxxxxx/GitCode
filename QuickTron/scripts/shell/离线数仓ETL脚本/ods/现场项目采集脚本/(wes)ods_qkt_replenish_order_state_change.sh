...
######### 设置表的变量
dbname=ods
table=ods_qkt_replenish_order_state_change
outdir=/data/sqoop/logs/hcatalog
target_dir=/warehouse/hive/ods/
mysql_dbname=evo_wes_replenish
mysql_table=replenish_order_state_change
...
###################### replenish_order_state_change->ods_qkt_replenish_order_state_change(T-1每日增量量->分区)
...
## 参数
columns=id,warehouse_id,replenish_order_id,state,remark,created_date,created_user,created_app,last_updated_date,last_updated_user,last_updated_app,project_code
...
use $dbname;
insert overwrite table dwd.dwd_replenish_order_state_change_info partition(d,pt)
select 
id,
warehouse_id,
replenish_order_id,
...
project_code as pt
from 
ods_qkt_replenish_order_state_change 
;
"
sql="
set hive.execution.engine=mr;
...
use $dbname;
insert overwrite table dwd.dwd_replenish_order_state_change_info partition(d='$pre1_date',pt)
select 
id,
warehouse_id,
replenish_order_id,
