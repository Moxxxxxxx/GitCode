...
#-- 功能描述 ： one工单变更记录
#-- 注意 ： 每日t-1分区
#-- 输入表 : dwd.dwd_ones_work_order_change_record_df、
#-- 输出表 ：dws.ods_qkt_ones_work_order_change_record_df
...
modify_user,
updated_time,
split(split(modify_content,'，')[0],'：')[1] as old_change_value,
split(split(modify_content,'，')[1],'：')[1] as new_change_value,
if(modify_content rlike '.*负责人.*' ,'负责人','案列状态') as order_change_type
from 
ods.ods_qkt_ones_work_order_change_record_df
where d='${pre1_date}' 
) t
) t1 ;
