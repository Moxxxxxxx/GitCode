...
left join ${dim_dbname}.dim_report_dashboard_slices_info e on if(nvl(a.slice_id,'')<>'',a.slice_id,'UNKNOWN')=e.slice_id
where a.d='2021-11-11' and (a.\`action\` in ('welcome','sql_json') or a.referrer like concat('http://superset.flashhold.com/superset/dashboard','%') or (a.action='explore_json' and nvl(a.dashboard_id,'')='' and a.slice_id>0))
;
