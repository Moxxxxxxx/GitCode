...
use $dbname;
insert overwrite table dwd.dwd_g2p_countcheck_work_detail_info partition(d,pt)
select 
id,
warehouse_id,
zone_code,
...
from
ods_qkt_g2p_countcheck_work_detail 
) t
...
,row_number() over(partition by id,project_code order by updated_date desc ) as rn 
from
ods_qkt_g2p_w2p_countcheck_work_detail  
) t
left join dim_project_product_type b on t.project_code=b.project_code
where t.rn=1
;
...
*
,row_number() over(partition by id,project_code order by updated_date desc) as rn
from ods_qkt_g2p_countcheck_work_detail
