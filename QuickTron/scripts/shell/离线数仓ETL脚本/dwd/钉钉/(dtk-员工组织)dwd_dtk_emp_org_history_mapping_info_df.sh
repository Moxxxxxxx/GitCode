...
#-- 功能描述 ： 钉钉员工历史组织信息记录表
#-- 注意 ： 每天存最新数据到昨天的分区内
#-- 输入表 : dim.dim_dtk_emp_org_mapping_info、ods.ods_qkt_dkt_dingtalk_user_info_df、dim.dim_dtk_org_history_info_df
...
from ${ods_dbname}.ods_qkt_dkt_dingtalk_user_info_df where d='${pre1_date}') b on a.emp_id=b.user_id 
...
from ${ods_dbname}.ods_qkt_dkt_dingtalk_user_info_df a
left join ${ods_dbname}.ods_qkt_dkt_dingtalk_user_info_df b on a.user_id=b.user_id 
and b.d='${pre1_date}'
