...
(
  ...
  (
    SELECT project_code,
           basic_units_qyt * 3600 as agv_actual_run_duration
    FROM ${tmp_dbname}.tmp_basic_live_data_offline_info
    WHERE basic_code = 0002
  )t1
...
(
  SELECT  t1.days as cur_date, -- 统计日期
          ...
          ((nvl(t3.agv_num,0) - nvl(t4.breakndown_agv_num,0)) + t8.oee) / nvl(t3.agv_num,0) as OEE, -- （无故障小车OEE + 有故障小车OEE）/ 小车总数
          (((nvl(t3.agv_num,0) - nvl(t4.breakndown_agv_num,0)) * t9.agv_actual_run_duration) + t8.mtbf) / nvl(t3.agv_num,0) as MTBF, -- （无故障小车MTBF + 有故障小车MTBF）/ 小车总数
  ...
  (
    ...
    (
      ...
      FROM ${dwd_dbname}.dwd_cyclecount_cycle_count_info cc
      LEFT JOIN ${dwd_dbname}.dwd_cyclecount_cycle_count_work_info ck
      ON cc.id = ck.cycle_count_id AND cc.d = ck.d AND cc.pt = ck.pt
      LEFT JOIN ${dwd_dbname}.dwd_g2p_countcheck_job_info cj
      ON ck.id = cj.work_id AND cc.d = cj.d AND cc.pt = cj.pt
      LEFT JOIN ${dwd_dbname}.dwd_rcs_basic_agv_info ba
      ON cj.agv_code = ba.agv_code AND cc.d = ba.d AND cc.pt = ba.pt
      LEFT JOIN ${dwd_dbname}.dwd_rcs_basic_agv_type_info bat 
    ...
    (
      ...
      (
        ...
        FROM ${dwd_dbname}.dwd_g2p_bucket_robot_job_info_di r
        LEFT JOIN ${dwd_dbname}.dwd_g2p_si_qp_extend_info e 
        ...
        LEFT JOIN ${dwd_dbname}.dwd_g2p_si_qp_extend_info e 
        ON r.project_code = e.project_code AND r.robot_job_id = e.job_id AND e.d = '${pre1_date}'
        LEFT JOIN ${dwd_dbname}.dwd_g2p_si_qp_transfer_job_info t
...
SELECT '' as id, -- 主键
       ...
       ((nvl(t1.agv_num,0) - nvl(t1.breakndown_agv_num,0)) + t2.oee) / nvl(t1.agv_num,0) as OEE, -- （无故障小车OEE + 有故障小车OEE）/ 小车总数
       (((nvl(t1.agv_num,0) - nvl(t1.breakndown_agv_num,0)) * t3.agv_actual_run_duration) + t2.mtbf) / nvl(t1.agv_num,0) as MTBF, -- （无故障小车MTBF + 有故障小车MTBF）/ 小车总数
...
json_name=(ads_project_view_breakdown_count.json ads_project_view_amr_type_detail.json ads_project_view_breakdown_trend.json)
