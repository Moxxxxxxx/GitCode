...
@uid:=t.robot_error_str AS robot_error_str_b
            from
                (select
                t1.id as error_id, t1.robot_code, t1.error_code, concat(t1.robot_code, '-', t1.error_code) as robot_error_str, t1.start_time, t1.end_time, t1.warning_spec, t1.alarm_module, t1.alarm_service, t1.alarm_type, t1.alarm_level, t3.alarm_name as alarm_detail, t1.param_value, t1.job_order, t1.robot_job, t1.device_code, t1.server_code, t1.transport_object, t1.point_location, substring_index(substring_index(t1.point_location, "x=", -1), ",", 1) as x, substring_index(substring_index(replace (t1.point_location, ")", ""), "y=", -1), ",", 1) as y, GREATEST(t1.start_time, @dt_week_start_time) as stat_start_time, case when t1.end_time is null or t1.end_time >= LEAST(@dt_next_week_start_time, @now_time) then LEAST(@dt_next_week_start_time, @now_time) else t1.end_time end as stat_end_time
                from (select *
           ...
           (select t.*,
            ...
            from (select t1.id                                                                                   as error_id,
                         ...
                         case
                             when t1.end_time is null or t1.end_time >= LEAST(@dt_next_week_start_time, @now_time)
                                 then LEAST(@dt_next_week_start_time, @now_time)
                  ...
                  from (select *
                        ...
                        where alarm_module = 'robot'
                          ...
                          and (
                                ...
                                (start_time >= @dt_week_start_time and start_time < @dt_next_week_start_time and
                                 coalesce(end_time, @now_time) >= @dt_next_week_start_time) or
                                (start_time < @dt_week_start_time and
                                 coalesce(end_time, @now_time) >= @dt_next_week_start_time and
                                 coalesce(end_time, @now_time) < @dt_next_week_start_time) or
                                (start_time < @dt_week_start_time and
                                 coalesce(end_time, @now_time) >= @dt_next_week_start_time)
                            )) t1
                           -- 注意：一定是用的inner join
                           inner join (select robot_code,
                                       ...
                                       where alarm_module = 'robot'
                                         ...
                                         and (
                                               (start_time >= @dt_week_start_time and
                                                start_time < @dt_next_week_start_time and
                                                coalesce(end_time, @now_time) < @dt_next_week_start_time) or
                                               (start_time >= @dt_week_start_time and
                                                start_time < @dt_next_week_start_time and
                                                coalesce(end_time, @now_time) >= @dt_next_week_start_time) or
