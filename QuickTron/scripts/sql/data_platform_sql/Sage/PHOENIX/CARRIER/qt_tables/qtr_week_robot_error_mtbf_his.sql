...
case when @dt_next_week_start_time <= @now_time then DATEDIFF(ms,@dt_week_start_time,coalesce(t3.the_week_first_create_time, @dt_next_week_start_time))/cast(1000 as decimal) else DATEDIFF(ms,@dt_week_start_time,coalesce(t3.the_week_first_create_time, @now_time))/cast(1000 as decimal) end as stat_state_duration  -- 每个机器人计算周之前的最后一条状态在该天内持续时长（秒）	
-- UNIX_TIMESTAMP(coalesce(t3.the_week_first_create_time, LEAST(@dt_next_week_start_time,@now_time)))-UNIX_TIMESTAMP(@dt_week_start_time) as stat_state_duration  -- 每个机器人计算周之前的最后一条状态在该天内持续时长（秒）					
from 
-- 找到每个机器人计算周之前的最后一条状态变化数据
(select 
...
case when t5.the_hour_last_id is not null and @dt_next_week_start_time <= @now_time then DATEDIFF(ms,t4.create_time,@dt_next_week_start_time)/cast(1000 as decimal) when t5.the_hour_last_id is not null and @dt_next_week_start_time > @now_time then DATEDIFF(ms,t4.create_time,@now_time)/cast(1000 as decimal) else t4.duration / cast(1000 as decimal) end as stat_state_duration  -- 每个机器人在计算周内的每条状态持续时长（最后一条要做特殊处理）（秒）
-- case when t5.the_hour_last_id is not null then UNIX_TIMESTAMP(LEAST(@dt_next_week_start_time,@now_time))-UNIX_TIMESTAMP(t4.create_time) else t4.duration / 1000 end as stat_state_duration  -- 每个机器人在计算周内的每条状态持续时长（最后一条要做特殊处理）（秒）
from 
-- 每个机器人计算周之内的状态变化数据
...
-- part3: 计算故障次数、故障时长
(select robot_code,
sum(DATEDIFF(ms,stat_start_time,stat_end_time)/cast(1000 as decimal)) as error_duration,  -- 该周故障时长
count(distinct error_id) as error_num  -- 该周故障次数
from 
(select
t1.error_id,
...
case when t1.start_time < @dt_week_start_time then @dt_week_start_time else t1.start_time end as stat_start_time,
case when t1.end_time is null or t1.end_time >= (case when @dt_next_week_start_time <= @now_time then @dt_next_week_start_time else @now_time end) then (case when @dt_next_week_start_time <= @now_time then @dt_next_week_start_time else @now_time end) else t1.end_time end as stat_end_time   -- 注意：算当前周期的指标值时，要考虑当前周期时间并没有全部过完这一客观事实
-- case when t1.end_time is null or t1.end_time >= @dt_next_week_start_time then LEAST(@dt_next_week_start_time,@now_time) else LEAST(t1.end_time,@now_time) end as stat_end_time  -- 注意：算当前周期的指标值时，要考虑当前周期时间并没有全部过完这一客观事实
from (select *
...
from 
(select robot_code,error_id
FROM qt_smartreport.dbo.qtr_week_robot_error_list_his
where date_value < @dt_week_start_time
union all 
select 
t1.robot_code,
