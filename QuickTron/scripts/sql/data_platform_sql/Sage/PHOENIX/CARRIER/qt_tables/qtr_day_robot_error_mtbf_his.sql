...
UNIX_TIMESTAMP(coalesce(t3.the_day_first_create_time, LEAST(@dt_next_day_start_time,@now_time)))-UNIX_TIMESTAMP(@dt_day_start_time) as stat_state_duration  -- 每个机器人计算天之前的最后一条状态在该天内持续时长（秒）					
from 
-- 找到每个机器人计算天之前的最后一条状态变化数据
(select 
robot_code, max(id) as before_the_hour_last_id 
from phoenix_rms.robot_state_history
where create_time < @dt_day_start_time
...
-- 找到每个机器人计算天之内的第一条状态变化数据
left join 
(select 
robot_code, min(create_time) as the_day_first_create_time
from phoenix_rms.robot_state_history
where create_time >= @dt_day_start_time and create_time < @dt_next_day_start_time
...
case when t5.the_hour_last_id is not null then UNIX_TIMESTAMP(LEAST(@dt_next_day_start_time,@now_time))-UNIX_TIMESTAMP(t4.create_time) else t4.duration / 1000 end as stat_state_duration  -- 每个机器人在计算小时内的每条状态持续时长（最后一条要做特殊处理）（秒）
from 
-- 每个机器人计算天之内的状态变化数据
(select 
*
from phoenix_rms.robot_state_history 
where create_time >= @dt_day_start_time and create_time < @dt_next_day_start_time )t4 
left join 
-- 找到每个机器人在计算天的最后一条状态变化数据
(select 
robot_code, 
...
-- part3: 计算故障次数、故障时长
(select robot_code,
sum(unix_timestamp(stat_end_time)-unix_timestamp(stat_start_time)) as error_duration,  -- 该天故障时长
count(distinct error_id) as error_num  -- 该天故障次数
from 
(select
t.error_id,
t.error_code,
...
case when t1.start_time < @dt_day_start_time then @dt_day_start_time else t1.start_time end as stat_start_time,
case when t1.end_time is null or t1.end_time >= @dt_next_day_start_time then LEAST(@dt_next_day_start_time,@now_time) else LEAST(t1.end_time,@now_time) end as stat_end_time   -- 注意：算当前周期的指标值时，要考虑当前周期时间并没有全部过完这一客观事实
from (select *
...
COALESCE(t1.theory_run_duration,0) as theory_run_duration -- 理论运行时长（秒） 
from 
(select distinct robot_code from phoenix_basic.dbo.basic_robot)br
left join 
(select 
ts.robot_code,
sum(stat_state_duration) as theory_run_duration
...
case when @dt_next_day_start_time <= @now_time then DATEDIFF(ms,@dt_day_start_time,coalesce(t3.the_day_first_create_time, @dt_next_day_start_time))/cast(1000 as decimal) else DATEDIFF(ms,@dt_day_start_time,coalesce(t3.the_day_first_create_time, @now_time))/cast(1000 as decimal) end as stat_state_duration  -- 每个机器人计算天之前的最后一条状态在该天内持续时长（秒）
-- UNIX_TIMESTAMP(coalesce(t3.the_day_first_create_time, LEAST(@dt_next_day_start_time,@now_time)))-UNIX_TIMESTAMP(@dt_day_start_time) as stat_state_duration  -- 每个机器人计算天之前的最后一条状态在该天内持续时长（秒）					
from 
-- 找到每个机器人计算天之前的最后一条状态变化数据
(select 
...
case when t5.the_hour_last_id is not null and @dt_next_day_start_time <= @now_time then DATEDIFF(ms,t4.create_time,@dt_next_day_start_time)/cast(1000 as decimal) when t5.the_hour_last_id is not null and @dt_next_day_start_time > @now_time then DATEDIFF(ms,t4.create_time,@now_time)/cast(1000 as decimal) else t4.duration / cast(1000 as decimal) end as stat_state_duration   -- 每个机器人在计算小时内的每条状态持续时长（最后一条要做特殊处理）（秒）
-- case when t5.the_hour_last_id is not null then UNIX_TIMESTAMP(LEAST(@dt_next_day_start_time,@now_time))-UNIX_TIMESTAMP(t4.create_time) else t4.duration / 1000 end as stat_state_duration  -- 每个机器人在计算小时内的每条状态持续时长（最后一条要做特殊处理）（秒）
from 
-- 每个机器人计算天之内的状态变化数据
(select * from phoenix_rms.dbo.robot_state_history 
where create_time >= @dt_day_start_time and create_time < @dt_next_day_start_time)t4 
left join 
-- 找到每个机器人在计算天的最后一条状态变化数据
(select 
...
-- part3: 计算故障次数、故障时长
(select robot_code,
sum(DATEDIFF(ms,stat_start_time,stat_end_time)/cast(1000 as decimal)) as error_duration,  -- 该天故障时长
count(distinct error_id) as error_num  -- 该天故障次数
from 
(select
t1.error_id,
...
case when t1.start_time < @dt_day_start_time then @dt_day_start_time else t1.start_time end as stat_start_time,
case when t1.end_time is null or t1.end_time >= (case when @dt_next_day_start_time <= @now_time then @dt_next_day_start_time else @now_time end) then (case when @dt_next_day_start_time <= @now_time then @dt_next_day_start_time else @now_time end) else t1.end_time end as stat_end_time   -- 注意：算当前周期的指标值时，要考虑当前周期时间并没有全部过完这一客观事实
-- case when t1.end_time is null or t1.end_time >= @dt_next_day_start_time then LEAST(@dt_next_day_start_time,@now_time) else LEAST(t1.end_time,@now_time) end as stat_end_time   -- 注意：算当前周期的指标值时，要考虑当前周期时间并没有全部过完这一客观事实
from (select *
...
from 
(select robot_code,error_id
FROM qt_smartreport.dbo.qtr_day_robot_error_list_his
where date_value < @dt_day_start_time
union all 
select 
t1.robot_code,
...
sum(theory_run_duration) as accum_theory_run_duration,  -- 该天之前累计理论运行时长
sum(error_duration) as accum_error_duration   -- 该天之前累计故障时长
from qt_smartreport.dbo.qtr_day_robot_error_mtbf_his
where date_value < @dt_day_start_time
group by robot_code)t4 on t4.robot_code=br.robot_code
...
from
-- 每个机器人计算天之内的状态变化数据
(select * from phoenix_rms.robot_state_history
where create_time >= {{ dt_day_start_time }} and create_time < {{ dt_next_day_start_time }})t4
left join
-- 找到每个机器人在计算天的最后一条状态变化数据
(select
