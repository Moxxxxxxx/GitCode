...
insert into qt_smartreport.qt_notification_robot_module_object_stat(time_value, date_value, hour_value, robot_code,
                                                           ...
                                                           add_notification_num, notification_num,
...
#time_type='小时' 
insert into qt_smartreport.qt_notification_robot_module_stat(time_value, date_value, hour_value, robot_num,
                                                             add_notification_num, notification_num,
                                                             notification_time, notification_rate, mtbf, mttr,
                                                             time_type)
select tt.hour_start_time                                                                      as time_value,
       date(tt.hour_start_time)                                                                as date_value,
       HOUR(tt.hour_start_time)                                                                as hour_value,
       ...
       coalesce(count(distinct t.notification_id), 0)                                          as notification_num,
       coalesce(sum(the_hour_cost_seconds), 0)                                                 as notification_time,
       cast(coalesce(sum(the_hour_cost_seconds), 0) / (3600 * tt.robot_num) as decimal(10, 4)) as notification_rate,
       cast(case
                when count(distinct t.notification_id) != 0 then cast(
...
select date_format(tt.hour_start_time, '%Y-%m-%d')                                                  as time_value,
       ...
       coalesce(count(distinct t.notification_id), 0)                                               as notification_num,
       coalesce(sum(the_hour_cost_seconds), 0)                                                      as notification_time,
       cast(coalesce(sum(the_hour_cost_seconds), 0) / (3600 * 24 * tt.robot_num) as decimal(10, 4)) as notification_rate,
       cast(case
                when count(distinct t.notification_id) != 0 then cast(
...
insert into qt_smartreport.qt_notification_robot_module_index_stat(time_value, date_value, hour_value, time_type,index_value, value_type)
select time_value,
