...
case when COALESCE(tas.estimate_distance,0)!=0 then (tas.rts_map_distance/1000)/(tas.estimate_distance/1000) end as detour_ratio,   -- 绕路距离比例
case when tja.is_loading = 0 then tas.rts_map_distance/1000 end as empty_move_distance,  -- 空车移动距离（米）
case when tja.is_loading = 0 then unix_timestamp(tas.lock_end_time) - unix_timestamp(tas.lock_start_time) end as empty_move_time,  -- 空车移动时长（秒）
case when tja.is_loading = 0 then (tas.rts_map_distance/1000)/(unix_timestamp(tas.lock_end_time) - unix_timestamp(tas.lock_start_time)) end as empty_move_speed,  -- 空车移动速度（米/秒）
case when tja.is_loading = 1 then tas.rts_map_distance/1000 end as loading_move_distance,  -- 带载移动距离（米）
case when tja.is_loading = 1 then unix_timestamp(tas.lock_end_time) - unix_timestamp(tas.lock_start_time) end as loading_move_time,  -- 带载移动时长（秒）
case when tja.is_loading = 1 then (tas.rts_map_distance/1000)/(unix_timestamp(tas.lock_end_time) - unix_timestamp(tas.lock_start_time)) end as loading_move_speed,  -- 带载移动速度（米/秒）
...
(
    ...
    `lock_start_time`           datetime(6)        DEFAULT NULL COMMENT '锁闭开始时间',	
    `lock_end_time`             datetime(6)        DEFAULT NULL COMMENT '锁闭结束时间',		
	...
	key idx_lock_end_time (`lock_end_time`)
)
    ENGINE = InnoDB
