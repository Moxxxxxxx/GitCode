...
select 
'day' as stat_time_type,
date_format({now_start_time}, '%Y-%m-%d 00:00:00') as stat_time_value,
br.robot_code,
brt.robot_type_code,
brt.robot_type_name,
...
case when {now_time} < {next_start_time} then UNIX_TIMESTAMP(coalesce(t3.the_hour_first_create_time, {now_time})) - UNIX_TIMESTAMP({now_start_time}) else UNIX_TIMESTAMP(coalesce(t3.the_hour_first_create_time, {next_start_time})) - UNIX_TIMESTAMP({now_start_time}) end stat_state_duration				
from 
(select 
...
case when t5.the_hour_last_id is not null and {now_time} >= {next_start_time} then UNIX_TIMESTAMP(@next_start_time)-UNIX_TIMESTAMP(t4.create_time)
when t5.the_hour_last_id is not null and {now_time} < {next_start_time} then UNIX_TIMESTAMP({now_time}) - UNIX_TIMESTAMP(t4.create_time)
      ...
      where alarm_module = 'robot'
        ...
        and (
              (start_time >= {now_start_time} and start_time < {next_start_time} and
               coalesce(end_time, {now_time}) < {next_start_time}) or
              (start_time >= {now_start_time} and start_time < {next_start_time} and
               coalesce(end_time, {now_time}) >= {next_start_time}) or
              (start_time < {now_start_time} and coalesce(end_time, {now_time}) >= {now_start_time} and
               coalesce(end_time, {now_time}) < {next_start_time}) or
              (start_time < {now_start_time} and coalesce(end_time, {now_time}) >= {next_start_time})
          )) t1
...
where date_value<{now_start_time}
union all 
...
where date_value=date(date_add({now_start_time}, interval -1 day)))t4  
