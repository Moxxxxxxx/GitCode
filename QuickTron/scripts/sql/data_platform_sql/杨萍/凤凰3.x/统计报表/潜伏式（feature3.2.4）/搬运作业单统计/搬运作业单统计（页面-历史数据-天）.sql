##########################################################################################
-- 搬运作业单链路
set @now_start_time = date_format(date_add(current_date(), interval -1 day), '%Y-%m-%d 00:00:00');
set @next_start_time = date_format(current_date(), '%Y-%m-%d 00:00:00');
select @now_start_time, @next_start_time;

set @now_start_time = date_format(current_date(), '%Y-%m-%d 00:00:00');
set @next_start_time = date_format(date_add(current_date(), interval 1 day), '%Y-%m-%d 00:00:00');
select @now_start_time, @next_start_time;
...
# step1:建表（qt_day_transport_order_link_detail_stat_his）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_day_transport_order_link_detail_stat_his
(
...
# step2:删除相关数据（qt_day_transport_order_link_detail_stat_his）
DELETE
...
# step3:插入当天相关数据（qt_day_transport_order_link_detail_stat_his）
insert into qt_smartreport.qt_day_transport_order_link_detail_stat_his(date_value,link_id, upstream_order_no, order_no,link_create_time,event_time, execute_state, order_state,robot_code, first_classification,robot_type_name,cost_time)
select 
