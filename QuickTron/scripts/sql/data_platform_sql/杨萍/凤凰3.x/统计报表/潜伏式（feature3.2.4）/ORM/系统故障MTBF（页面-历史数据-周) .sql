# step1:建表（qt_week_sys_error_list_his）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_week_sys_error_list_his
(
    `id`                   bigint(20)   NOT NULL AUTO_INCREMENT COMMENT '主键',
    `date_value`           date         NOT NULL COMMENT '日期（周的星期一）',
    ...
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='系统周维度故障结果集（T+1）';
			


# step2:删除相关数据（qt_week_sys_error_list_his）
DELETE
...
# step3:插入相关数据（qt_week_sys_error_list_his）
insert into qt_smartreport.qt_week_sys_error_list_his(date_value, error_id, error_code, start_time, end_time,warning_spec,alarm_module,alarm_service,alarm_type,alarm_level,alarm_detail,param_value,job_order,robot_job,robot_code,device_code,server_code,transport_object)
select 
...
# step4:建表（qt_week_sys_error_mtbf_his）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_week_sys_error_mtbf_his
(
    `id`                        bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键',
    `date_value`                date       NOT NULL COMMENT '日期（周的星期一）',
    ...
    COLLATE = utf8_general_ci
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='系統周维度mtbf（T+1）';
	
	

# step5:删除相关数据（qt_week_sys_error_mtbf_his）
DELETE
...
# step6:插入相关数据（qt_week_sys_error_mtbf_his）
insert into qt_smartreport.qt_week_sys_error_mtbf_his(date_value,alarm_service,theory_run_duration,error_duration,error_num,mtbf,accum_theory_run_duration,accum_error_duration,accum_error_num,accum_mtbf)	
select 
DATE_SUB(CURRENT_DATE(),INTERVAL WEEKDAY(CURRENT_DATE()) +7 DAY) as date_value,  -- 上周星期一
ts.alarm_service,
