...
where hour_start_time >=  {now_start_time}  and hour_start_time <  {next_start_time} 
union all
select
...
-- part2: 前一天的累计理论运行时长、累计故障时长（去重）
left join
(select
alarm_service,accum_theory_run_duration,accum_error_duration
from qt_smartreport.qt_day_sys_error_mtbf_his
where date_value=date_add(current_date(), interval -1 day))t2 on t2.alarm_service=ts.alarm_service
-- part3: 当天参与计算的故障数、累计参与计算的故障数
left join
(select
COALESCE(alarm_service,'ALL_SYS') as alarm_service_name,
count(distinct case when date_value=CURRENT_DATE() then error_id end) as error_num,
...
union all
select date_value,alarm_service ,error_id
from qt_smartreport.qt_hour_sys_error_list_his
where hour_start_time >=  {now_start_time}  and hour_start_time <  {next_start_time} 
union all
select date( {now_hour_start_time} ) as date_value,alarm_service ,id as error_id
from phoenix_basic.basic_notification
where alarm_module in ('system', 'server')
  and alarm_level >= 3
  and (
...
WITH ROLLUP)t3 on t3.alarm_service_name=ts.alarm_service)t 
