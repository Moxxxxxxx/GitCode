...
select
       ...
       tbr.robot_type_code,
       tbr.robot_type_name,
	   case when date_format(sysdate(), '%Y-%m-%d %H:00:00')=tbr.hour_start_time then COALESCE(t1.uptime_state_duration, 0)/(UNIX_TIMESTAMP(sysdate())-UNIX_TIMESTAMP(tbr.hour_start_time)) else COALESCE(t1.uptime_state_duration, 0) / 3600 end as uptime_state_rate,
       COALESCE(t1.uptime_state_duration, 0)                                        as uptime_state_duration,
       case when date_format(sysdate(), '%Y-%m-%d %H:00:00')=tbr.hour_start_time then UNIX_TIMESTAMP(sysdate())-UNIX_TIMESTAMP(tbr.hour_start_time) else 3600 end as uptime_state_rate_fenmu,   
	   case when date_format(sysdate(), '%Y-%m-%d %H:00:00')=tbr.hour_start_time then COALESCE(t1.loading_busy_state_duration, 0) /  (UNIX_TIMESTAMP(sysdate())-UNIX_TIMESTAMP(tbr.hour_start_time)) else  COALESCE(t1.loading_busy_state_duration, 0) / 3600 end as utilization_rate,
       COALESCE(t1.loading_busy_state_duration, 0)                                  as utilization_duration,
       case when date_format(sysdate(), '%Y-%m-%d %H:00:00')=tbr.hour_start_time then UNIX_TIMESTAMP(sysdate())-UNIX_TIMESTAMP(tbr.hour_start_time) else 3600 end as utilization_rate_fenmu,
	   ...
	   case when date_format(sysdate(), '%Y-%m-%d %H:00:00')=tbr.hour_start_time then  (UNIX_TIMESTAMP(sysdate())-UNIX_TIMESTAMP(tbr.hour_start_time)) - COALESCE(t1.loading_busy_state_duration, 0) - COALESCE(t1.empty_busy_state_duration, 0) -
       COALESCE(t1.charging_state_duration, 0) - COALESCE(t1.idle_state_duration, 0) -
       COALESCE(t1.locked_state_duration, 0) - COALESCE(t1.error_state_duration, 0) else
       3600 - COALESCE(t1.loading_busy_state_duration, 0) - COALESCE(t1.empty_busy_state_duration, 0) -
       COALESCE(t1.charging_state_duration, 0) - COALESCE(t1.idle_state_duration, 0) -
       COALESCE(t1.locked_state_duration, 0) - COALESCE(t1.error_state_duration, 0) end as offline_duration
