...
brt.robot_type_code,
brt.robot_type_name,
COALESCE(max(create_robot_error_num), 0) as create_robot_error_num,
COALESCE(max(create_order_num), 0)       as create_order_num,
COALESCE(max(create_job_num), 0)         as create_job_num,
COALESCE(max(robot_run_time), 0)         as robot_run_time,
COALESCE(max(robot_error_time), 0)       as robot_error_time,
COALESCE(max(end_robot_error_num), 0)    as end_robot_error_num,
COALESCE(max(end_robot_error_time), 0)   as end_robot_error_time
from
(
...
group by tb.hour_start_time,tb.robot_code
...
group by trt.hour_start_time,trt.robot_code)tr
