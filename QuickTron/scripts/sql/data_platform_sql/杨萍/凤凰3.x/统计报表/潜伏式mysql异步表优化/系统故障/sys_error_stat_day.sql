...
-- part1:所筛选天范围内理论运行时长、故障时长、故障次数、累计理论运行时长、累计故障时长、累计故障次数
from qt_smartreport.qtr_day_sys_error_mtbf_his t1
-- part2:筛选天范围内的新增故障次数、结束故障次数、结束故障时间、时间段内持续的故障次数
left join
(select
t1.date_value,
COALESCE(t1.alarm_service,'ALL_SYS') as alarm_service_name,
count(distinct case when date(t1.start_time)=t1.date_value then t1.error_id end) as create_sys_error_num,  -- 新增故障次数
count(distinct case when t1.end_time is not null and date(t1.end_time)=t1.date_value then t1.error_id end) as end_sys_error_num,  -- 结束故障次数
sum(case when t1.end_time is not null and date(t1.end_time)=t1.date_value then unix_timestamp(t1.end_time)-unix_timestamp(t1.start_time) end) as end_error_time -- 已结束故障时长（秒）
from
...
-- part3:筛选天范围内的新增订单量、新增任务量
left join
(select
date(tor.create_time) as date_value,
