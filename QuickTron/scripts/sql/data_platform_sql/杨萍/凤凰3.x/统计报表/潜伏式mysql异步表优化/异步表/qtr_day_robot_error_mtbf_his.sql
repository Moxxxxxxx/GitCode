...
UNIX_TIMESTAMP(coalesce(t3.the_hour_first_create_time, LEAST(@dt_next_day_start_time,@now_time)))-UNIX_TIMESTAMP(@dt_day_start_time) as stat_state_duration  -- 每个机器人计算天之前的最后一条状态在该天内持续时长（秒）					
from 
...
-- 插入数据（异步表）qt_smartreport.qtr_day_robot_error_mtbf_his	
-- {{ dt_relative_time(dt) }}
-- {{ now_time }}
-- {{ dt_hour_start_time }}
