...
from
(
-- part1:筛选周机器人新增故障次数、结束故障次数、已结束故障时长
select
tb.robot_code,
tb.date_value,
count(distinct case when date(DATE_SUB(tb.start_time,INTERVAL WEEKDAY(tb.start_time) + 0 DAY))=tb.date_value then tb.error_id end) as create_robot_error_num,   -- 新增故障次数
count(distinct case when tb.end_time is not null and date(DATE_SUB(tb.end_time,INTERVAL WEEKDAY(tb.end_time) + 0 DAY))=tb.date_value then tb.error_id end) as end_robot_error_num, -- 结束故障次数
sum(case when tb.end_time is not null and date(DATE_SUB(tb.end_time,INTERVAL WEEKDAY(tb.end_time) + 0 DAY))=tb.date_value then unix_timestamp(tb.end_time)-unix_timestamp(tb.start_time) end) as end_robot_error_time,  -- 已结束故障时长（秒）
...
from qt_smartreport.qtr_week_robot_error_list_his t
...
-- part2:所筛选周机器人理论运行时长、机器人故障时长、机器人故障次数、机器人累计理论运行时长、机器人累计故障时长、机器人累计故障次数
union all
select
robot_code,
date_value,
