...
where alarm_module in ('system', 'server')
  and alarm_level >= 3
        and (
              ...
              (start_time >= @dt_day_start_time and start_time < @dt_next_day_start_time and coalesce(end_time, @now_time) >= @dt_next_day_start_time) or
              (start_time < @dt_day_start_time and coalesce(end_time, @now_time) >= @dt_day_start_time and coalesce(end_time, @now_time) < @dt_next_day_start_time) or
...
-- 插入数据（异步表）qt_smartreport.qtr_day_sys_error_detail_his	
-- {{ dt_relative_time(dt) }}
-- {{ now_time }}
-- {{ dt_hour_start_time }}
