...
where hour_start_time>= @dt_week_start_time and hour_start_time <  @dt_next_week_start_time
...
where date_value >= @dt_week_start_time and date_value <  @dt_next_week_start_time
...
from qt_smartreport.qtr_week_sys_error_list_his
where date_value < @dt_next_week_start_time
group by alarm_service
WITH rollup)t3 on t3.alarm_service_name=ts.alarm_service
-- 各系统该周前一周累计理论运行时长、累计故障时长（时间段上去重）
left join  
...
-- 插入数据（异步表）qt_smartreport.qtr_week_sys_error_mtbf_his	
-- {{ dt_relative_time(dt) }}
-- {{ now_time }}
-- {{ dt_hour_start_time }}
