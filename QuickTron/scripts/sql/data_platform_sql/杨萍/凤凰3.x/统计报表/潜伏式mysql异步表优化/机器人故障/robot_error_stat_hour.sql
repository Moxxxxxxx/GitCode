...
date_format(tr.hour_start_time, '%Y-%m-%d %H:00:00') as stat_time_value,
tr.robot_code,
brt.robot_type_code,
brt.robot_type_name,
...
from
(
-- part1:筛选小时机器人新增故障次数、结束故障次数、已结束故障时长
select
tb.robot_code,
tb.hour_start_time,
count(distinct case when date_format(tb.start_time,'%Y-%m-%d %H:00:00')=tb.hour_start_time then tb.error_id end) as create_robot_error_num,   -- 新增故障次数
count(distinct case when tb.end_time is not null and date_format(tb.end_time,'%Y-%m-%d %H:00:00')=tb.hour_start_time then tb.error_id end) as end_robot_error_num, -- 结束故障次数
sum(case when tb.end_time is not null and date_format(tb.end_time,'%Y-%m-%d %H:00:00')=tb.hour_start_time then unix_timestamp(tb.end_time)-unix_timestamp(tb.start_time) end) as end_robot_error_time,  -- 已结束故障时长（秒）
...
group by tb.robot_code,tb.hour_start_time
-- part2:所筛选小时机器人理论运行时长、机器人故障时长、机器人故障次数、机器人累计理论运行时长、机器人累计故障时长、机器人累计故障次数
union all
select
robot_code,
hour_start_time,
...
where hour_start_time between { start_time } and { end_time })tr
