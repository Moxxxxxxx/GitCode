...
where enter_charging_power is not null
and coalesce(enter_charging_time,bind_robot_time) >= @dt_day_start_time  and  coalesce(enter_charging_time,bind_robot_time) < @dt_next_day_start_time
group by FORMAT(cast(coalesce(enter_charging_time,bind_robot_time) as datetime), 'yyyy-MM-dd HH:00:00'),charger_code
union all 
select 
...
where enter_charging_power is not null
and recover_charger_time >= @dt_day_start_time and recover_charger_time < @dt_next_day_start_time
group by FORMAT(cast(recover_charger_time as datetime), 'yyyy-MM-dd HH:00:00'),charger_code)t
left join phoenix_basic.dbo.basic_charger bc on bc.charger_code=t.charger_code
