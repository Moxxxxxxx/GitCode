##step1:建表（qt_robot_state_history_next_step1）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_robot_state_history_next_step1
(
    id         bigint(20),
...
##step2:删除相关数据（qt_robot_state_history_next_step1）
DELETE FROM qt_smartreport.qt_robot_state_history_next_step1;



##step3:插入相关数据（qt_robot_state_history_next_step1）
insert into qt_smartreport.qt_robot_state_history_next_step1(id,robot_code,current_id,next_id)
SELECT id,
       robot_code,
...
##step4:建表（qt_robot_state_history_next_step2）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_robot_state_history_next_step2
(
    id         bigint(20),
    robot_code varchar(32),
    next_id    varchar(32),
...
##step5:删除相关数据（qt_robot_state_history_next_step2）
DELETE FROM qt_smartreport.qt_robot_state_history_next_step2;


##step6:插入相关数据（qt_robot_state_history_next_step2）
insert into qt_smartreport.qt_robot_state_history_next_step2(id,robot_code,next_id)
SELECT t1.id, t1.robot_code, t2.id as next_id
from qt_smartreport.qt_robot_state_history_next_step1 t1
         LEFT JOIN qt_smartreport.qt_robot_state_history_next_step1 t2
                   ON t1.next_id = t2.current_id;



##step7:建表（qt_robot_state_time_hour_detail）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_robot_state_time_hour_detail
(
    ...
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='机器人状态在小时内耗时明细';	





##step8:删除当天相关数据（qt_robot_state_time_hour_detail）
DELETE
FROM qt_smartreport.qt_robot_state_time_hour_detail
WHERE date(hour_start_time) = date(date_add(sysdate(), interval -1 day));  
...
##step9:插入当天相关数据（qt_robot_state_time_hour_detail）
insert into qt_smartreport.qt_robot_state_time_hour_detail(hour_start_time, next_hour_start_time, state_id,
...
from (select th.day_hours                               as hour_start_time,
      ...
      from (SELECT DATE_FORMAT(DATE_SUB(DATE_FORMAT(date(date_add(sysdate(), interval -1 day)), '%Y-%m-%d 00:00:00'), INTERVAL
         ...
         inner join(select t.*,
                           case
                               when t.create_time < date_format(date(date_add(sysdate(), interval -1 day)), '%Y-%m-%d 00:00:00')
                                   then date_format(date(date_add(sysdate(), interval -1 day)), '%Y-%m-%d 00:00:00')
                               else t.create_time end as start_time,
                           case
                               when t.next_time >= date_format(date_add(date(date_add(sysdate(), interval -1 day)), interval 1 day), '%Y-%m-%d 00:00:00')
                                   then date_format(date_add(date(date_add(sysdate(), interval -1 day)), interval 1 day), '%Y-%m-%d 00:00:00')
                    ...
                    from (select t1.id,
                          ...
                          from phoenix_rms.robot_state_history t1
                                   left join qt_smartreport.qt_robot_state_history_next_step2 tm
                          ...
                          where date(t1.create_time) >= date(date_add(date(date_add(sysdate(), interval -1 day)), interval -10 day))) t
                    where 1 = 1
                      and ((t.create_time >= date_format(date(date_add(sysdate(), interval -1 day)), '%Y-%m-%d 00:00:00') and
                            t.create_time < date_format(date_add(date(date_add(sysdate(), interval -1 day)), interval 1 day), '%Y-%m-%d 00:00:00') and
                            t.next_time < date_format(date_add(date(date_add(sysdate(), interval -1 day)), interval 1 day), '%Y-%m-%d 00:00:00'))
                        or
                           (t.create_time >= date_format(date(date_add(sysdate(), interval -1 day)), '%Y-%m-%d 00:00:00') and
                            t.create_time < date_format(date_add(date(date_add(sysdate(), interval -1 day)), interval 1 day), '%Y-%m-%d 00:00:00') and
                            t.next_time >= date_format(date_add(date(date_add(sysdate(), interval -1 day)), interval 1 day), '%Y-%m-%d 00:00:00'))
                        or
                           (t.create_time < date_format(date(date_add(sysdate(), interval -1 day)), '%Y-%m-%d 00:00:00') and
                            t.next_time >= date_format(date(date_add(sysdate(), interval -1 day)), '%Y-%m-%d 00:00:00') and
                            t.next_time < date_format(date_add(date(date_add(sysdate(), interval -1 day)), interval 1 day), '%Y-%m-%d 00:00:00'))
                        or
                           (t.create_time < date_format(date(date_add(sysdate(), interval -1 day)), '%Y-%m-%d 00:00:00') and
                            t.next_time >= date_format(date_add(date(date_add(sysdate(), interval -1 day)), interval 1 day), '%Y-%m-%d 00:00:00')))) t2
                   on (
...
-----------------------------------------------------------------------
##step10:建表(qt_robot_state_time_stat)
...
##step11:删除当天相关数据(qt_robot_state_time_stat)
DELETE
FROM qt_smartreport.qt_robot_state_time_stat
WHERE date(time_value) = date(date_add(sysdate(), interval -1 day));  




##step12-1:插入当天相关数据(qt_robot_state_time_stat)
##time_type='小时'
...
----------------------------------
##step13:建表(qt_robot_state_time_rate_stat)
...
##step14:删除数据(qt_robot_state_time_rate_stat)
DELETE
...
##step15:插入当天相关数据(qt_robot_state_time_rate_stat)
