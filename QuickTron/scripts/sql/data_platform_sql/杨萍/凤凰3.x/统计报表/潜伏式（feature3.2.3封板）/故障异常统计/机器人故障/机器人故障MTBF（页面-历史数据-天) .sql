...
# step1:建表（qt_day_robot_error_list_his）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_day_robot_error_list_his
(
    ...
    `stat_start_time`      datetime(6)           DEFAULT NULL COMMENT '天内参与计算的开始时间',
    `stat_end_time`        datetime(6)           DEFAULT NULL COMMENT '结束时间-天内参与计算的结束时间',
    ...
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='机器人天维度故障收敛结果集（T+1）';
			


# step2:删除相关数据（qt_day_robot_error_list_his）
DELETE
...
# step3:插入相关数据（qt_day_robot_error_list_his）
insert into qt_smartreport.qt_day_robot_error_list_his(date_value, error_id, error_code, start_time, end_time,warning_spec,alarm_module,alarm_service,alarm_type,alarm_level,alarm_detail,param_value,job_order,robot_job,robot_code,device_code,server_code,transport_object,stat_start_time,stat_end_time)
select 
...
# step4:建表（qt_day_robot_error_mtbf_his）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_day_robot_error_mtbf_his
(
    `id`                        bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键',
    `date_value`                date       NOT NULL COMMENT '日期',
    ...
    COLLATE = utf8_general_ci
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='天维度机器人mtbf（T+1）';
	
	

# step5:删除相关数据（qt_day_robot_error_mtbf_his）
DELETE
...
# step6:插入相关数据（qt_day_robot_error_mtbf_his）
insert into qt_smartreport.qt_day_robot_error_mtbf_his(date_value,robot_code,theory_run_duration,error_duration,error_num,mtbf,accum_theory_run_duration,accum_error_duration,accum_error_num,accum_mtbf)	
select 
date_add(CURRENT_DATE(), interval -1 day) as date_value,
br.robot_code,
COALESCE(t1.theory_run_duration,0) as theory_run_duration,
