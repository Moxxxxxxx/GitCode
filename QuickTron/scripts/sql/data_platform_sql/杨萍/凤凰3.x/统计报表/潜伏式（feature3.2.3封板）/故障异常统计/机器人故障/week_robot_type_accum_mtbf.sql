...
date_format(t.date_value, '%Y-%m-%d 00:00:00') as stat_time_value,
t.robot_code,
brt.robot_type_code,
brt.robot_type_name,
...
from qt_smartreport.qt_week_robot_error_mtbf_his t
...
select 
'week' as stat_time_type,
date_format({now_week_start_time}, '%Y-%m-%d 00:00:00') as stat_time_value,
br.robot_code,
brt.robot_type_code,
brt.robot_type_name,
...
case when {now_time} < {now_next_week_start_time} then UNIX_TIMESTAMP(coalesce(t3.the_hour_first_create_time, {now_time})) - UNIX_TIMESTAMP({now_week_start_time}) else UNIX_TIMESTAMP(coalesce(t3.the_hour_first_create_time, {now_next_week_start_time})) - UNIX_TIMESTAMP({now_week_start_time}) end stat_state_duration				
from 
(select 
robot_code, max(id) as before_the_hour_last_id 
from phoenix_rms.robot_state_history
where create_time < {now_week_start_time}
...
from phoenix_rms.robot_state_history
where create_time >= {now_week_start_time} and create_time < {now_next_week_start_time}
...
case when t5.the_hour_last_id is not null and {now_time} >= {now_next_week_start_time} then UNIX_TIMESTAMP({now_next_week_start_time})-UNIX_TIMESTAMP(t4.create_time)
when t5.the_hour_last_id is not null and {now_time} < {now_next_week_start_time} then UNIX_TIMESTAMP({now_time}) - UNIX_TIMESTAMP(t4.create_time)
else t4.duration / 1000 end stat_state_duration
from 
(select 
*
from phoenix_rms.robot_state_history 
where create_time >= {now_week_start_time} and create_time < {now_next_week_start_time})t4 
left join 
(select 
robot_code, 
...
(select 
       ...
       t1.server_code,
       t1.transport_object,
	   case when t1.start_time < {now_week_start_time} then {now_week_start_time} else t1.start_time end as stat_start_time,
	   case when t1.end_time is null or t1.end_time >= {now_next_week_start_time} then {now_next_week_start_time} else t1.end_time end as stat_end_time
from (select *
      from phoenix_basic.basic_notification
      where alarm_module = 'robot'
        and alarm_level >= 3
        and (
              (start_time >= {now_week_start_time} and start_time < {now_next_week_start_time} and
               coalesce(end_time, {now_time}) < {now_next_week_start_time}) or
              (start_time >= {now_week_start_time} and start_time < {now_next_week_start_time} and
               coalesce(end_time, {now_time}) >= {now_next_week_start_time}) or
              (start_time < {now_week_start_time} and coalesce(end_time, {now_time}) >= {now_week_start_time} and
               coalesce(end_time, {now_time}) < {now_next_week_start_time}) or
              (start_time < {now_week_start_time} and coalesce(end_time, {now_time}) >= {now_next_week_start_time})
          )) t1
...
where date_value<{now_week_start_time}
union all 
...
set @now_week_start_time = date_format(DATE_SUB(CURRENT_DATE(),INTERVAL WEEKDAY(CURRENT_DATE()) + 0 DAY), '%Y-%m-%d 00:00:00');
set @now_next_week_start_time = date_format(DATE_SUB(CURRENT_DATE(),INTERVAL WEEKDAY(CURRENT_DATE()) - 7 DAY), '%Y-%m-%d 00:00:00');
select @now_week_start_time, @now_next_week_start_time;
...
t2.is_error, 
t2.duration / 1000 as                           duration,
case when sysdate() < @now_next_week_start_time then UNIX_TIMESTAMP(coalesce(t3.the_hour_first_create_time, sysdate())) - UNIX_TIMESTAMP(@now_week_start_time) else UNIX_TIMESTAMP(coalesce(t3.the_hour_first_create_time, @now_next_week_start_time)) - UNIX_TIMESTAMP(@now_week_start_time) end stat_state_duration				
from 
(select 
robot_code, max(id) as before_the_hour_last_id 
from phoenix_rms.robot_state_history
...
t4.is_error, 
t4.duration / 1000 as           duration,
case when t5.the_hour_last_id is not null and sysdate() >= @now_next_week_start_time then UNIX_TIMESTAMP(@now_next_week_start_time)-UNIX_TIMESTAMP(t4.create_time)
when t5.the_hour_last_id is not null and sysdate() < @now_next_week_start_time then UNIX_TIMESTAMP(sysdate()) - UNIX_TIMESTAMP(t4.create_time)
else t4.duration / 1000 end stat_state_duration
from 
(select 
*
...
from (select *
      ...
      where alarm_module = 'robot'
        ...
        and (
              (start_time >= @now_week_start_time and start_time < @now_next_week_start_time and
               coalesce(end_time, sysdate()) < @now_next_week_start_time) or
              (start_time >= @now_week_start_time and start_time < @now_next_week_start_time and
               coalesce(end_time, sysdate()) >= @now_next_week_start_time) or
              (start_time < @now_week_start_time and coalesce(end_time, sysdate()) >= @now_week_start_time and
               coalesce(end_time, sysdate()) < @now_next_week_start_time) or
              (start_time < @now_week_start_time and coalesce(end_time, sysdate()) >= @now_next_week_start_time)
          )) t1
         inner join (select robot_code,
                            COALESCE(end_time, '未结束') as end_time,
                            min(id)                      as first_error_id
