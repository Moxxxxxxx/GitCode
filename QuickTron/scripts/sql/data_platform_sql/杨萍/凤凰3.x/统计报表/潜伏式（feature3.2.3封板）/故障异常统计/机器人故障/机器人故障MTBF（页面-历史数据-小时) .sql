...
# step1:建表（qt_hour_robot_error_list_his）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_hour_robot_error_list_his
(
...
# step2:删除相关数据（qt_hour_robot_error_list_his）
DELETE
...
# step3:插入相关数据（qt_hour_robot_error_list_his）
insert into qt_smartreport.qt_hour_robot_error_list_his(date_value,hour_start_time,next_hour_start_time, error_id, error_code, start_time, end_time,warning_spec,alarm_module,alarm_service,alarm_type,alarm_level,alarm_detail,param_value,job_order,robot_job,robot_code,device_code,server_code,transport_object,stat_start_time,stat_end_time)
select 
...
(
    ...
    COLLATE = utf8_general_ci
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='小时维度机器人mtbf（H+1）';
