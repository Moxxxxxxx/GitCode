...
(select alarm_service,
       ...
       start_time as original_start_time,
       end_time as original_end_time,
	   case when start_time<@now_hour_start_time then date_format(DATE_ADD(sysdate(), INTERVAL -1 HOUR), '%Y-%m-%d %H:00:00') else start_time end start_time,
	   case when COALESCE(end_time,sysdate())>=@now_next_hour_start_time then date_format(sysdate(), '%Y-%m-%d %H:00:00') else COALESCE(end_time,sysdate()) end as end_time
from qt_smartreport.qt_hour_sys_error_list_his
where hour_start_time = date_format(DATE_ADD(sysdate(), INTERVAL -1 HOUR), '%Y-%m-%d %H:00:00')
order by alarm_service,original_start_time asc)t)t
left join (
