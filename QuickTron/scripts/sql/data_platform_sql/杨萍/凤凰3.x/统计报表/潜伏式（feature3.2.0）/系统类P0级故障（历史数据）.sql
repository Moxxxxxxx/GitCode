------------------------------------------------------------------------------------------------
--step1:建表（qt_notification_system_module_p0_time_hour_detail）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_notification_system_module_p0_time_hour_detail
(
    ...
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='系统类P0级故障通知在小时内耗时明细';	




------------------------------------------------------------------------------------------------
--step2:删除当天相关数据（qt_notification_system_module_p0_time_hour_detail）
DELETE
...
------------------------------------------------------------------------------------------------
--step3:插入当天相关数据(qt_notification_system_module_p0_time_hour_detail)

insert into qt_smartreport.qt_notification_system_module_p0_time_hour_detail(hour_start_time,next_hour_start_time,notification_id,error_code,alarm_module,alarm_service,alarm_type,alarm_level,robot_code,first_classification_name,job_order,the_hour_order_seconds,device_code,server_code,object_code,object_type,start_time,end_time,the_hour_cost_seconds)
            ...
            where 1 = 1
              ...
              and (((bn.start_time >= date_format(date_add(sysdate(), interval -1 day), '%Y-%m-%d 00:00:00') and
                ...
                or
                    (bn.start_time < date_format(date_add(sysdate(), interval -1 day), '%Y-%m-%d 00:00:00') and
                     date_format(coalesce(bn.end_time, sysdate()), '%Y-%m-%d %H:%i:%s') >=
                     date_format(sysdate(), '%Y-%m-%d 00:00:00'))) or bn.end_time is null)) t2 on 1) ta
         left join
            ...
            where 1 = 1
              and ((create_time >= date_format(date_add(sysdate(), interval -1 day), '%Y-%m-%d 00:00:00') and
                ...
                or
                   (create_time < date_format(date_add(sysdate(), interval -1 day), '%Y-%m-%d 00:00:00') and
                    date_format(coalesce(update_time, sysdate()), '%Y-%m-%d %H:%i:%s') >=
                    date_format(sysdate(), '%Y-%m-%d 00:00:00')))) t2 on 1
...
--------------------------------
##step4:建表(qt_notification_system_module_p0_object_stat)
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_notification_system_module_p0_object_stat
(
    `id`                        int(20)   NOT NULL AUTO_INCREMENT COMMENT '主键',
    ...
    COLLATE = utf8_general_ci
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='系统类P0告警通知在时间段内指标统计';	
	
	
--------------------------------
##step5:删除当天相关数据(qt_notification_system_module_p0_object_stat)
DELETE
...
--------------------------------
##step6-1:插入当天相关数据(qt_notification_system_module_p0_object_stat)
#time_type='小时'
#alarm_module = 'system' and alarm_type = 'robot'

insert into qt_smartreport.qt_notification_system_module_p0_object_stat(time_value, date_value, hour_value, object_code,object_type,first_classification_name,add_notification_num, notification_num,notification_time, notification_rate, notification_rate_fenzi,notification_rate_fenmu,mtbf,mtbf_fenzi,mtbf_fenmu, mttr,mttr_fenzi,mttr_fenmu, time_type)
...
from (select t1.hour_start_time,
      ...
      from (select th.day_hours                               as hour_start_time,
         ...
         left join qt_smartreport.qt_notification_system_module_p0_time_hour_detail t
...
--------------------------------
##step6-2:插入当天相关数据(qt_notification_system_module_p0_object_stat)
#time_type='小时'
...
--------------------------------
##step6-3:插入当天相关数据(qt_notification_system_module_p0_object_stat)
#time_type='小时'
...
--------------------------------
##step6-4:插入当天相关数据(qt_notification_system_module_p0_object_stat)
#time_type='小时'
         ...
         left join qt_smartreport.qt_notification_system_module_p0_time_hour_detail t
                   on t.alarm_module = 'server' and t.alarm_service = 'SERVER' and
                      ...
                      t.server_code = tt.server_code and date_format(t.hour_start_time, '%Y-%m-%d') =
                       ...
                       and t.the_hour_cost_seconds is not null
group by time_value, date_value, hour_value, tt.server_code
;


---------------------------------------------------------------------------------------
##step6-5:插入当天相关数据(qt_notification_system_module_p0_object_stat)
#time_type='天'
#alarm_module = 'system' and alarm_type = 'robot'
...
---------------------------------------------------------------------------------------
##step6-6:插入当天相关数据(qt_notification_system_module_p0_object_stat)
#time_type='天'
#alarm_module = 'system' and alarm_type = 'device'
...
------------------------------------------------------------
##step6-7:插入当天相关数据(qt_notification_system_module_p0_object_stat)
#time_type='天'
#alarm_module = 'system' and alarm_type = 'job'
...
-----------------------------------------------------------
##step6-8:插入当天相关数据(qt_notification_system_module_p0_object_stat)
#time_type='天'
#alarm_module = 'server' and alarm_service = 'SERVER'
...
------------------------------------------------------------------------------------------
--------------------------------
##step7:建表(qt_notification_system_module_p0_stat)
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_notification_system_module_p0_stat
(
    `id`                         int(20)   NOT NULL AUTO_INCREMENT COMMENT '主键',
    `time_value`                 datetime  NOT NULL COMMENT '统计时间',
    `date_value`                 date               DEFAULT NULL COMMENT '日期',
    ...
    COLLATE = utf8_general_ci
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='全场系统类P0级告警通知在时间段内指标统计';	
	
	
--------------------------------
##step8:删除当天相关数据(qt_notification_system_module_p0_stat)
DELETE
FROM qt_smartreport.qt_notification_system_module_p0_stat
WHERE date_format(time_value, '%Y-%m-%d') = date_format(date_add(sysdate(), interval -1 day), '%Y-%m-%d');  


--------------------------------
##step9-1:插入当天相关数据(qt_notification_system_module_p0_stat)
#time_type='小时'

insert into qt_smartreport.qt_notification_system_module_p0_stat(time_value, date_value, hour_value, object_num,robot_num, device_num, server_num, order_num,the_hour_order_seconds_sum, add_notification_num,notification_num, notification_time, notification_rate,notification_rate_fenzi,notification_rate_fenmu,mtbf,mtbf_fenzi,mtbf_fenmu, mttr,mttr_fenzi,mttr_fenmu, time_type)
...
------------------------------------------------------------------------------------------
##step9-2:插入当天相关数据(qt_notification_system_module_p0_stat)
#time_type='天'
...
##step10:建表(qt_notification_system_module_p0_index_stat)
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_notification_system_module_p0_index_stat
(
    `id`                        int(20)   NOT NULL AUTO_INCREMENT COMMENT '主键',
    `time_value`                datetime  NOT NULL COMMENT '统计时间',
    `date_value`                date               DEFAULT NULL COMMENT '日期',
    ...
    COLLATE = utf8_general_ci
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='全场系统类P0级告警通知在时间段内各指标值';	
	
	
--------------------------------
##step11:删除数据(qt_notification_system_module_p0_index_stat)
DELETE
...
--------------------------------
##step12:插入当天相关数据(qt_notification_system_module_p0_index_stat)

insert into qt_smartreport.qt_notification_system_module_p0_index_stat(time_value, date_value, hour_value, time_type,index_value,index_value_fenzi,index_value_fenmu, value_type)
