##step1:建表（临时表 qt_robot_charge_detail_temp）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_robot_charge_detail_temp
(
    ...
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='机器人充电明细临时表';	




--------------------------------
##step2:删除当天相关数据（临时表 qt_robot_charge_detail_temp）
DELETE
FROM qt_smartreport.qt_robot_charge_detail_temp
...
--------------------------------
##step3:插入当天相关数据(临时表 qt_robot_charge_detail_temp)
insert into qt_smartreport.qt_robot_charge_detail_temp(robot_code, first_classification_name,
...
where date_format(t.end_charge_time, '%Y-%m-%d') = date_format(date_add(sysdate(), interval -1 day), '%Y-%m-%d')
group by robot_code, first_classification_name, charge_state_id, charger_code, end_charge_time;



--------------------------------
##step4:建表(qt_robot_charge_stat)
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_robot_charge_stat
(
    `id`                        int(20)   NOT NULL AUTO_INCREMENT COMMENT '主键',
    ...
    COLLATE = utf8_general_ci
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='机器人充电统计表';	
	

--------------------------------
##step5:删除当天相关数据(qt_robot_charge_stat)
DELETE
...
##step6:插入当天相关数据(qt_robot_charge_stat)
insert into qt_smartreport.qt_robot_charge_stat(time_value, date_value, hour_value, robot_code,
...
##step7:建表(qt_charger_charge_stat)
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_charger_charge_stat
(
    `id`            int(20)   NOT NULL AUTO_INCREMENT COMMENT '主键',
    `time_value`    datetime  NOT NULL COMMENT '统计时间',
    `date_value`    date               DEFAULT NULL COMMENT '日期',
    ...
    COLLATE = utf8_general_ci
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='充电桩充电统计表';	
	
	
	

--------------------------------
##step8:删除当天相关数据(qt_charger_charge_stat)
DELETE
...
##step9:插入当天相关数据(qt_charger_charge_stat)
insert into qt_smartreport.qt_charger_charge_stat(time_value, date_value, hour_value, charger_code, charge_num,
