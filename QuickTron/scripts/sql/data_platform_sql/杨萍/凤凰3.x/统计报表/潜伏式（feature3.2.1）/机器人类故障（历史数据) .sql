------------------------------------------------------------------------------------------------
--step1:建表（qt_notification_robot_module_time_hour_detail）
...
------------------------------------------------------------------------------------------------
--step2:删除当天相关数据（qt_notification_robot_module_time_hour_detail）
DELETE
...
------------------------------------------------------------------------------------------------
--step3:插入当天相关数据(qt_notification_robot_module_time_hour_detail)
insert into qt_smartreport.qt_notification_robot_module_time_hour_detail(hour_start_time, next_hour_start_time,
...
select t1.hour_start_time,
       ...
       t2.end_time,		   
       case
           when t2.start_time >= t1.hour_start_time and t2.start_time < t1.next_hour_start_time and coalesce(t2.end_time, sysdate()) < t1.next_hour_start_time then UNIX_TIMESTAMP(coalesce(t2.end_time, sysdate()))-UNIX_TIMESTAMP(t2.start_time)
           when t2.start_time >= t1.hour_start_time and t2.start_time < t1.next_hour_start_time and coalesce(t2.end_time, sysdate()) >= t1.next_hour_start_time then UNIX_TIMESTAMP(t1.next_hour_start_time)-UNIX_TIMESTAMP(t2.start_time)
           when t2.start_time < t1.hour_start_time and coalesce(t2.end_time, sysdate()) >= t1.hour_start_time and coalesce(t2.end_time, sysdate()) < t1.next_hour_start_time then  UNIX_TIMESTAMP(coalesce(t2.end_time, sysdate()))-UNIX_TIMESTAMP(t1.hour_start_time)
           when t2.start_time < t1.hour_start_time and coalesce(t2.end_time, sysdate()) >= t1.next_hour_start_time then  UNIX_TIMESTAMP(t1.next_hour_start_time)-UNIX_TIMESTAMP(t1.hour_start_time)
                                       ...
                                       from (select t1.id         as error_id,
                                             ...
                                             from (select b1.*
                                                   from phoenix_basic.basic_notification b1
                                                            inner join (SELECT robot_code,
                                                                        ...
                                                                        where bn.alarm_module = 'robot'
																		  ...
																		  and (bn.start_time >= date_format(date_add(sysdate(), interval -10 day),'%Y-%m-%d 00:00:00') or bn.end_time is null or bn.end_time >= date_format(date_add(sysdate(), interval -10 day),'%Y-%m-%d 00:00:00'))
                                                   ...
                                                   where 1 = 1
													 ...
													 and (b1.start_time >= date_format(date_add(sysdate(), interval -10 day),'%Y-%m-%d 00:00:00') or b1.end_time is null or b1.end_time >= date_format(date_add(sysdate(), interval -10 day),'%Y-%m-%d 00:00:00'))
                                                  ...
                                                  (select b2.*
                                                   from phoenix_basic.basic_notification b2
                                                            inner join (SELECT robot_code,
                                                                        ...
                                                                        where bn.alarm_module = 'robot'
                                                                          and bn.alarm_level >= 3
																		  and (start_time >= date_format(date_add(sysdate(), interval -10 day),'%Y-%m-%d 00:00:00') or end_time is null or end_time >= date_format(date_add(sysdate(), interval -10 day),'%Y-%m-%d 00:00:00'))
                                                   ...
                                                   where 1 = 1
													 ...
													 and (b2.start_time >= date_format(date_add(sysdate(), interval -10 day),'%Y-%m-%d 00:00:00') or b2.end_time is null or b2.end_time >= date_format(date_add(sysdate(), interval -10 day),'%Y-%m-%d 00:00:00'))
...
--------------------------------
##step4:建表(qt_notification_robot_module_object_stat)
...
--------------------------------
##step5:删除当天相关数据(qt_notification_robot_module_object_stat)
DELETE
...
select tt.hour_start_time                                                                      as time_value,
       ...
       coalesce(sum(the_hour_cost_seconds), 0) / 3600                                          as notification_rate,
...
--------------------------------
##step6-2:插入当天相关数据(qt_notification_robot_module_object_stat)
#time_type='天'
...
----------------------------------------------------------------------------------!!!!!!!以下数据集后期慢慢不用了！！！！！！！！------------------------------------------------------------------------
--------------------------------
##step7:建表(qt_notification_robot_module_stat)
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_notification_robot_module_stat
(
    `id`                        int(20)   NOT NULL AUTO_INCREMENT COMMENT '主键',
    `time_value`                datetime  NOT NULL COMMENT '统计时间',
    `date_value`                date               DEFAULT NULL COMMENT '日期',
    ...
    COLLATE = utf8_general_ci
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='全场机器人类告警通知在时间段内指标统计';	
	
	
--------------------------------
##step8:删除当天相关数据(qt_notification_robot_module_stat)
DELETE
FROM qt_smartreport.qt_notification_robot_module_stat
WHERE date_format(time_value, '%Y-%m-%d') = date_format(date_add(sysdate(), interval -1 day), '%Y-%m-%d');  


--------------------------------
##step9-1:插入当天相关数据(qt_notification_robot_module_stat)
#time_type='小时' 
insert into qt_smartreport.qt_notification_robot_module_stat(time_value, date_value, hour_value, robot_num,add_notification_num, notification_num,notification_time, notification_rate,notification_rate_fenzi,notification_rate_fenmu, mtbf,mtbf_fenzi,mtbf_fenmu,mttr,mttr_fenzi,mttr_fenmu,time_type)
      ...
      group by t1.hour_start_time, t1.next_hour_start_time) tt
         left join qt_smartreport.qt_notification_robot_module_time_hour_detail t
                   on t.hour_start_time = tt.hour_start_time and t.next_hour_start_time = tt.next_hour_start_time and
                      ...
                      date_format(date_add(sysdate(), interval -1 day), '%Y-%m-%d') and t.the_hour_cost_seconds is not null 
...
--------------------------------
##step9-2:插入当天相关数据(qt_notification_robot_module_stat)
#time_type='天'
...
--------------------------------
##step10:建表(qt_notification_robot_module_index_stat)
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_notification_robot_module_index_stat
(
    `id`                        int(20)   NOT NULL AUTO_INCREMENT COMMENT '主键',
    `time_value`                datetime  NOT NULL COMMENT '统计时间',
    `date_value`                date               DEFAULT NULL COMMENT '日期',
    ...
    COLLATE = utf8_general_ci
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='全场机器人类告警通知在时间段内各指标值';	
	
	
--------------------------------
##step11:删除数据(qt_notification_robot_module_index_stat)
DELETE
...
--------------------------------
##step12:插入当天相关数据(qt_notification_robot_module_index_stat)
insert into qt_smartreport.qt_notification_robot_module_index_stat(time_value, date_value, hour_value, time_type,index_value,index_value_fenzi,index_value_fenmu, value_type)
select time_value,
...
from (select th.day_hours                               as hour_start_time,
     ...
     (select bn.robot_code,
      ...
      from phoenix_basic.basic_notification bn
          ...
          select t.*
          from (select tn.*,
                       tp.end_time                                       as prev_end_time,
                       timestampdiff(second, tp.end_time, tn.start_time) as diff_seconds,
                       case
                           when tn.prev_error_id is null then 1
                           when timestampdiff(second, tp.end_time, tn.start_time) < 60 then 0
                           else 1 end                                       is_effective
                from (select t.*,
                             max(t.before_error_id)   as prev_error_id,
                             max(t.before_start_time) as prev_start_time
                      from (select t1.id         as error_id,
                            ...
                            from (select *
                                  ...
                                  where 1 = 1
                                    ...
                                    and start_time >=
                                        date_format(date_add(sysdate(), interval -2 day), '%Y-%m-%d 00:00:00')
                                 ) t1
                                     left join
                                 (select *
                                  from phoenix_basic.basic_notification
                                  where 1 = 1
                                    ...
                                    and start_time >=
                                        date_format(date_add(sysdate(), interval -2 day), '%Y-%m-%d 00:00:00')
                                 ) t2 on t2.robot_code = t1.robot_code  and t2.error_code = t1.error_code  and t2.start_time < t1.start_time) t
                      group by 1, 2, 3, 4, 5,6,7,8) tn
                         left join phoenix_basic.basic_notification tp on tp.id = tn.prev_error_id) t
          where t.is_effective = 1
      ) tbn on tbn.error_id = bn.id
      where 1 = 1
        and bn.alarm_module = 'robot'
        and ((bn.start_time >= date_format(date_add(sysdate(), interval -1 day), '%Y-%m-%d 00:00:00') and
              bn.start_time < date_format(sysdate(), '%Y-%m-%d 00:00:00') and
...
--历史版本2：故障等级>=3&结束时间相同取第一条&1分钟收敛规则
select t1.hour_start_time,
       t1.next_hour_start_time,
       t2.robot_code,
       t2.first_classification_name,
     ...
     (select bn.robot_code,
      ...
      from phoenix_basic.basic_notification bn
               ...
               inner join (select t.*
                           from (select tn.*,
                                        b.end_time                                       as prev_end_time,
                                        timestampdiff(second, b.end_time, tn.start_time) as diff_seconds,
                                        case
                                            when tn.prev_error_id is null then 1
                                            when timestampdiff(second, b.end_time, tn.start_time) < 60 then 0
                                            else 1 end                                      is_effective
                                 from (select t.error_id,
                                              t.robot_code,
                                              t.start_time,
...
---历史版本3：故障等级>=3&结束时间相同取第一条&1分钟收敛规则 &时间差计算精确到ms
select t1.hour_start_time,
       t1.next_hour_start_time,
       t2.robot_code,
       t2.first_classification_name,
