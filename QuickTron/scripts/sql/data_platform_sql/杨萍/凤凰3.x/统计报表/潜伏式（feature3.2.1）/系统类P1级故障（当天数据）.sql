------------------------------------------------------------------------------------------------
--step1:建表（qt_notification_system_module_p1_time_hour_detail_realtime）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_notification_system_module_p1_time_hour_detail_realtime
(
    ...
    `the_hour_order_seconds`    decimal(10, 4)     DEFAULT NULL COMMENT '作业单在该小时内时长（秒）',
    ...
    `the_hour_cost_seconds`     decimal(10, 4)     DEFAULT NULL COMMENT '在该小时内时长（秒）',
    ...
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='系统类P1级故障通知在小时内耗时明细（当天数据）';	




------------------------------------------------------------------------------------------------
--step2:删除当天相关数据（qt_notification_system_module_p1_time_hour_detail_realtime）
DELETE
FROM qt_smartreport.qt_notification_system_module_p1_time_hour_detail_realtime;  



------------------------------------------------------------------------------------------------
--step3:插入当天相关数据(qt_notification_system_module_p1_time_hour_detail_realtime)

insert into qt_smartreport.qt_notification_system_module_p1_time_hour_detail_realtime(hour_start_time,next_hour_start_time,notification_id,error_code,alarm_module,alarm_service,alarm_type,alarm_level,robot_code,first_classification_name,job_order,the_hour_order_seconds,device_code,server_code,object_code,object_type,start_time,end_time,the_hour_cost_seconds)
            ...
            where 1 = 1
              ...
              and (((bn.start_time >= date_format(sysdate(), '%Y-%m-%d 00:00:00') and
                ...
                or
                    (bn.start_time < date_format(sysdate(), '%Y-%m-%d 00:00:00') and
                     date_format(coalesce(bn.end_time, sysdate()), '%Y-%m-%d %H:%i:%s') >=
                     sysdate())) or bn.end_time is null)) t2 on 1) ta
         left join
     (select t1.hour_start_time,
      ...
      from (select h.hour_start_time,
           ...
           (select order_no,
            ...
            where 1 = 1
              and ((create_time >= date_format(sysdate(), '%Y-%m-%d 00:00:00') and
                    create_time < sysdate() and
                ...
                or
                   (create_time < date_format(sysdate(), '%Y-%m-%d 00:00:00') and
                    date_format(coalesce(update_time, sysdate()), '%Y-%m-%d %H:%i:%s') >=
                    sysdate()))) t2 on 1
      having the_hour_order_seconds is not null) tb
     on tb.order_no = ta.job_order and tb.hour_start_time = ta.hour_start_time and
...
--------------------------------
##step4:建表(qt_notification_system_module_p1_object_stat_realtime)
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_notification_system_module_p1_object_stat_realtime
(
    `id`                        int(20)   NOT NULL AUTO_INCREMENT COMMENT '主键',
    ...
    `notification_time`         decimal(10, 4)           DEFAULT NULL COMMENT '告警时长（秒）',
    `notification_rate`         decimal(10, 4)     DEFAULT NULL COMMENT '告警率',
    `notification_rate_fenzi`   decimal(10, 4)           DEFAULT NULL COMMENT '告警率分子',
    `notification_rate_fenmu`   decimal(10, 4)           DEFAULT NULL COMMENT '告警率分母',
    `mtbf`                      decimal(10, 4)     DEFAULT NULL COMMENT 'mtbf',
    `mtbf_fenzi`                decimal(10, 4)           DEFAULT NULL COMMENT 'mtbf分子',
    `mtbf_fenmu`                decimal(10, 4)           DEFAULT NULL COMMENT 'mtbf分母',
    `mttr`                      decimal(10, 4)     DEFAULT NULL COMMENT 'mttr',
    `mttr_fenzi`                decimal(10, 4)           DEFAULT NULL COMMENT 'mttr分子',
    `mttr_fenmu`                decimal(10, 4)           DEFAULT NULL COMMENT 'mttr分母',
    ...
    COLLATE = utf8_general_ci
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='系统类P1告警通知在时间段内指标统计（当天数据）';	
	
	
--------------------------------
##step5:删除当天相关数据(qt_notification_system_module_p1_object_stat_realtime)
DELETE
FROM qt_smartreport.qt_notification_system_module_p1_object_stat_realtime;  




--------------------------------
##step6-1:插入当天相关数据(qt_notification_system_module_p1_object_stat_realtime)
#time_type='小时'
#alarm_module = 'system' and alarm_type = 'robot'

insert into qt_smartreport.qt_notification_system_module_p1_object_stat_realtime(time_value, date_value, hour_value, object_code,object_type,first_classification_name,add_notification_num, notification_num,notification_time, notification_rate, notification_rate_fenzi,notification_rate_fenmu,mtbf,mtbf_fenzi,mtbf_fenmu, mttr,mttr_fenzi,mttr_fenmu, time_type)
...
select tt.hour_start_time                                 as time_value,
       ...
       coalesce(sum(t.the_hour_cost_seconds), 0) / 3600   as notification_rate,
       coalesce(sum(t.the_hour_cost_seconds), 0)          as notification_rate_fenzi,
       3600                                               as notification_rate_fenmu,
       case
           when count(distinct t.notification_id) != 0 then (3600 - coalesce(sum(t.the_hour_cost_seconds), 0)) /
                                                            count(distinct t.notification_id)
           else 3600 end                                  as mtbf,
       (3600 - coalesce(sum(t.the_hour_cost_seconds), 0)) as mtbf_fenzi,
       coalesce(count(distinct t.notification_id), 0)     as mtbf_fenmu,
       case
           when count(distinct t.notification_id) != 0 then coalesce(sum(t.the_hour_cost_seconds), 0) /
                                                            count(distinct t.notification_id)
           else 0 end                                     as mttr,
       coalesce(sum(t.the_hour_cost_seconds), 0)          as mttr_fenzi,
...
from (select t1.hour_start_time,
      ...
      from (select h.hour_start_time,
         ...
         left join qt_smartreport.qt_notification_system_module_p1_time_hour_detail_realtime t
...
--------------------------------
##step6-2:插入当天相关数据(qt_notification_system_module_p1_object_stat_realtime)
#time_type='小时'
            ...
            from (select th.day_hours                               as hour_start_time,
                  ...
                  from (SELECT DATE_FORMAT(DATE_SUB(DATE_FORMAT(sysdate(), '%Y-%m-%d 00:00:00'), INTERVAL
                        ...
                        FROM (SELECT a
                             ...
                             (SELECT @u := -1) AS i) th) h
            where h.hour_start_time <= sysdate()) t1
               inner join phoenix_basic.basic_equipment t2 on 1) tt
         left join qt_smartreport.qt_notification_system_module_p1_time_hour_detail_realtime t
...
--------------------------------
##step6-3:插入当天相关数据(qt_notification_system_module_p1_object_stat_realtime)
#time_type='小时'
...
select tt.hour_start_time                                                  as time_value,
       ...
       coalesce(sum(t.the_hour_cost_seconds), 0)                           as notification_rate_fenzi,
       coalesce(sum(tt.the_hour_order_seconds), 0)                         as notification_rate_fenmu,
       case
           when count(distinct t.notification_id) != 0 then
                   ...
                   count(distinct t.notification_id)
           else coalesce(sum(tt.the_hour_order_seconds), 0) end            as mtbf,
       (coalesce(sum(tt.the_hour_order_seconds), 0) -
        coalesce(sum(t.the_hour_cost_seconds), 0))                         as mtbf_fenzi,
       coalesce(count(distinct t.notification_id), 0)                      as mtbf_fenmu,
       case
           when count(distinct t.notification_id) != 0 then coalesce(sum(t.the_hour_cost_seconds), 0) /
                                                            count(distinct t.notification_id)
...
from (select t1.hour_start_time,
      ...
      from (select h.hour_start_time,
           ...
           (select order_no            as order_id,
                   order_type,
                   dispatch_robot_code as robot_code,
                   create_time         as start_time,
                   update_time         as end_time,
                   order_state
            from phoenix_rss.transport_order
...
--------------------------------
##step6-4:插入当天相关数据(qt_notification_system_module_p1_object_stat_realtime)
#time_type='小时'
            ...
            from (select th.day_hours                               as hour_start_time,
                  ...
                  from (SELECT DATE_FORMAT(DATE_SUB(DATE_FORMAT(sysdate(), '%Y-%m-%d 00:00:00'), INTERVAL
                        ...
                        FROM (SELECT a
                             ...
                             (SELECT @u := -1) AS i) th) h
            where h.hour_start_time <= sysdate()) t1
               inner join (select distinct server_code
...
---------------------------------------------------------------------------------------
##step6-5:插入当天相关数据(qt_notification_system_module_p1_object_stat_realtime)
#time_type='天'
#alarm_module = 'system' and alarm_type = 'robot'
...
select date_format(tt.hour_start_time, '%Y-%m-%d')           as time_value,
       ...
       coalesce(sum(the_hour_cost_seconds), 0) / (3600 * 24) as notification_rate,
...
---------------------------------------------------------------------------------------
##step6-6:插入当天相关数据(qt_notification_system_module_p1_object_stat_realtime)
#time_type='天'
#alarm_module = 'system' and alarm_type = 'device'
...
------------------------------------------------------------
##step6-7:插入当天相关数据(qt_notification_system_module_p1_object_stat_realtime)
#time_type='天'
#alarm_module = 'system' and alarm_type = 'job'
...
select date_format(tt.hour_start_time, '%Y-%m-%d')                                             as time_value,
       ...
       coalesce(count(distinct t.notification_id), 0)                                          as notification_num,
       coalesce(sum(the_hour_cost_seconds), 0)                                                 as notification_time,
       coalesce(sum(the_hour_cost_seconds), 0) / coalesce(sum(tt.the_hour_order_seconds), 0)   as notification_rate,
       coalesce(sum(the_hour_cost_seconds), 0)                                                 as notification_rate_fenzi,
       coalesce(sum(tt.the_hour_order_seconds), 0)                                             as notification_rate_fenmu,
       case
           when count(distinct t.notification_id) != 0 then
       ...
       (coalesce(sum(tt.the_hour_order_seconds), 0) - coalesce(sum(the_hour_cost_seconds), 0)) as mtbf_fenzi,
       coalesce(count(distinct t.notification_id), 0)                                          as mtbf_fenmu,
       case
           when count(distinct t.notification_id) != 0 then coalesce(sum(the_hour_cost_seconds), 0) /
                                                            count(distinct t.notification_id)
...
-----------------------------------------------------------
##step6-8:插入当天相关数据(qt_notification_system_module_p1_object_stat_realtime)
#time_type='天'
#alarm_module = 'server' and alarm_service = 'SERVER'
...
------------------------------------------------------------------------------------------
##step7:建表(qt_notification_system_module_p1_stat_realtime)
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_notification_system_module_p1_stat_realtime
(
    `id`                         int(20)   NOT NULL AUTO_INCREMENT COMMENT '主键',
    `time_value`                 datetime  NOT NULL COMMENT '统计时间',
    `date_value`                 date               DEFAULT NULL COMMENT '日期',
    ...
    `notification_rate`          decimal(10, 4)     DEFAULT NULL COMMENT '告警率',
    `notification_rate_fenzi`    int(100)           DEFAULT NULL COMMENT '告警率分子',
    `notification_rate_fenmu`    int(100)           DEFAULT NULL COMMENT '告警率分母',
    `mtbf`                       decimal(10, 2)     DEFAULT NULL COMMENT 'mtbf',
    `mtbf_fenzi`                 int(100)           DEFAULT NULL COMMENT 'mtbf分子',
    `mtbf_fenmu`                 int(100)           DEFAULT NULL COMMENT 'mtbf分母',
    `mttr`                       decimal(10, 2)     DEFAULT NULL COMMENT 'mttr',
    `mttr_fenzi`                 int(100)           DEFAULT NULL COMMENT 'mttr分子',
    `mttr_fenmu`                 int(100)           DEFAULT NULL COMMENT 'mttr分母',
    `time_type`                  varchar(100)       DEFAULT NULL COMMENT '统计维度',
    `created_time`               timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_time`               timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`)
    ...
    COLLATE = utf8_general_ci
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='全场系统类P1级告警通知在时间段内指标统计（当天数据）';	
	
	
--------------------------------
##step8:删除当天相关数据(qt_notification_system_module_p1_stat_realtime)
DELETE
FROM qt_smartreport.qt_notification_system_module_p1_stat_realtime;  


--------------------------------
##step9-1:插入当天相关数据(qt_notification_system_module_p1_stat_realtime)
#time_type='小时'

insert into qt_smartreport.qt_notification_system_module_p1_stat_realtime(time_value, date_value, hour_value, object_num,robot_num, device_num, server_num, order_num,the_hour_order_seconds_sum, add_notification_num,notification_num, notification_time, notification_rate,notification_rate_fenzi,notification_rate_fenmu,mtbf,mtbf_fenzi,mtbf_fenmu, mttr,mttr_fenzi,mttr_fenmu, time_type)
...
select tt.hour_start_time                                                                                as time_value,
       ...
       coalesce(count(distinct t.notification_id), 0)                                                    as notification_num,
       coalesce(sum(the_hour_cost_seconds), 0)                                                           as notification_time,
       cast(coalesce(sum(the_hour_cost_seconds), 0) / (3600 * (coalesce(tt.robot_num,0) + tt.device_num + tt.server_num) + tt.the_hour_order_seconds_sum) as decimal(10, 4)) as notification_rate,
	   coalesce(sum(the_hour_cost_seconds), 0) as notification_rate_fenzi,
	   (3600 * (coalesce(tt.robot_num,0) + tt.device_num + tt.server_num) + tt.the_hour_order_seconds_sum) as notification_rate_fenmu,
       cast(case when count(distinct t.notification_id) != 0 then cast(((3600 * (coalesce(tt.robot_num,0) + tt.device_num + tt.server_num) + tt.the_hour_order_seconds_sum) -coalesce(sum(the_hour_cost_seconds), 0)) /count(distinct t.notification_id) as decimal(10, 2)) else 3600 * (coalesce(tt.robot_num,0)+ tt.device_num + tt.server_num) + tt.the_hour_order_seconds_sum end as decimal(10, 2))                                as mtbf,
	   case when count(distinct t.notification_id) != 0 then ((3600 * (coalesce(tt.robot_num,0) + tt.device_num + tt.server_num) + tt.the_hour_order_seconds_sum) -coalesce(sum(the_hour_cost_seconds), 0)) else 3600 * (coalesce(tt.robot_num,0)+ tt.device_num + tt.server_num) + tt.the_hour_order_seconds_sum end  as mtbf_fenzi,
	   coalesce(count(distinct t.notification_id),0) as mtbf_fenmu,   
...
------------------------------------------------------------------------------------------
##step9-2:插入当天相关数据(qt_notification_system_module_p1_stat_realtime)
#time_type='天'
...
select date_format(tt.hour_start_time, '%Y-%m-%d')                                                                    as time_value,
       ...
       coalesce(count(distinct t.notification_id), 0)                                                                 as notification_num,
       coalesce(sum(the_hour_cost_seconds), 0)                                                                        as notification_time,
       cast(coalesce(sum(the_hour_cost_seconds), 0) / (3600 * 24 * (coalesce(tt.robot_num,0) + tt.device_num + tt.server_num) + coalesce(t1.the_hour_order_seconds_sum, 0)) as decimal(10, 4)) as notification_rate,
	   coalesce(sum(the_hour_cost_seconds), 0) as notification_rate_fenzi,
	   (3600 * 24 * (coalesce(tt.robot_num,0) + tt.device_num + tt.server_num) + coalesce(t1.the_hour_order_seconds_sum, 0)) as notification_rate_fenmu,
       cast(case when count(distinct t.notification_id) != 0 then cast(((3600 * 24 * (coalesce(tt.robot_num,0) + tt.device_num + tt.server_num) + coalesce(t1.the_hour_order_seconds_sum, 0)) - coalesce(sum(the_hour_cost_seconds), 0)) / count(distinct t.notification_id) as decimal(10, 2)) else 3600 * 24 * (coalesce(tt.robot_num,0) + tt.device_num + tt.server_num) + coalesce(t1.the_hour_order_seconds_sum, 0) end as decimal(10, 2))                                as mtbf,
	   case when count(distinct t.notification_id) != 0 then ((3600 * 24 * (coalesce(tt.robot_num,0) + tt.device_num + tt.server_num) + coalesce(t1.the_hour_order_seconds_sum, 0)) - coalesce(sum(the_hour_cost_seconds), 0)) else 3600 * 24 * (coalesce(tt.robot_num,0) + tt.device_num + tt.server_num) + coalesce(t1.the_hour_order_seconds_sum, 0) end as mtbf_fenzi,
	   coalesce(count(distinct t.notification_id),0) as mtbf_fenmu,
       cast(case when count(distinct t.notification_id) != 0 then cast(coalesce(sum(the_hour_cost_seconds), 0) / count(distinct t.notification_id) as decimal(10, 2)) else 0 end as decimal(10, 2)) as mttr,
	   coalesce(sum(the_hour_cost_seconds), 0) as mttr_fenzi,
	   coalesce(count(distinct t.notification_id),0) as mttr_fenmu,
                        ...
                        FROM (SELECT a
                              FROM (SELECT '1' AS a UNION SELECT '2' UNION SELECT '3' UNION SELECT '4') AS a
                                       JOIN(SELECT '1'
                                            ...
                                            SELECT '6') AS b ON 1) AS b,
                             (SELECT @u := -1) AS i) th) h
            where h.hour_start_time <= sysdate()) t1) tt
         left join qt_smartreport.qt_notification_system_module_p1_time_hour_detail_realtime t
                   on t.hour_start_time = tt.hour_start_time and t.next_hour_start_time = tt.next_hour_start_time and
                      date_format(t.hour_start_time, '%Y-%m-%d') =
                      date_format(sysdate(), '%Y-%m-%d')
         left join (select date_format(tt.hour_start_time, '%Y-%m-%d') as time_value,
                    ...
                    from (select t1.hour_start_time,
                          ...
                          from (select h.hour_start_time,
                                ...
                                from (select th.day_hours                               as hour_start_time,
                                             DATE_ADD(th.day_hours, INTERVAL 60 MINUTE) as next_hour_start_time
                                      from (SELECT DATE_FORMAT(DATE_SUB(DATE_FORMAT(sysdate(), '%Y-%m-%d 00:00:00'),
                                                                        INTERVAL
                                                                        (-(@ho := @ho + 1)) HOUR),
                                                               '%Y-%m-%d %H:00:00') as day_hours
                                            FROM (SELECT a
...
##step10:建表(qt_notification_system_module_p1_index_stat_realtime)
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_notification_system_module_p1_index_stat_realtime
(
    `id`                        int(20)   NOT NULL AUTO_INCREMENT COMMENT '主键',
    `time_value`                datetime  NOT NULL COMMENT '统计时间',
    `date_value`                date               DEFAULT NULL COMMENT '日期',
    ...
    COLLATE = utf8_general_ci
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='全场系统类P1级告警通知在时间段内各指标值（当天数据）';	
	
	
--------------------------------
##step11:删除数据(qt_notification_system_module_p1_index_stat_realtime)
DELETE
FROM qt_smartreport.qt_notification_system_module_p1_index_stat_realtime;   
	

--------------------------------
##step12:插入当天相关数据(qt_notification_system_module_p1_index_stat_realtime)

insert into qt_smartreport.qt_notification_system_module_p1_index_stat_realtime(time_value, date_value, hour_value, time_type,index_value,index_value_fenzi,index_value_fenmu, value_type)
