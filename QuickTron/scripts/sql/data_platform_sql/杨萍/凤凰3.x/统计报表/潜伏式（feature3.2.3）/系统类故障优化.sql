select alarm_module, module, alarm_type, warning_spec, count(*)
from phoenix_basic.basic_error_info
where alarm_module in ('device', 'server', 'system')
group by alarm_module, module, alarm_type, warning_spec



select warning_spec,count(0)
from phoenix_basic.basic_notification
where alarm_module in ('device','server','system')
and alarm_level in (3,4,5,6)
group by warning_spec


-------------------------------------------------------------

set @do_date = date(date_add('2022-08-03', interval -1 day));

select @do_date,
date_format(@do_date, '%Y-%m-%d 00:00:00'),
date_format(date_add(@do_date, interval 1 day), '%Y-%m-%d 00:00:00')
;
...
where 1 = 1
  ...
  and ((
               (bn.start_time >= date_format(@do_date, '%Y-%m-%d 00:00:00') and
                bn.start_time < date_format(date_add(@do_date, interval 1 day), '%Y-%m-%d 00:00:00') and
                coalesce(bn.end_time, sysdate()) < date_format(date_add(@do_date, interval 1 day), '%Y-%m-%d 00:00:00'))
               or
               (bn.start_time >= date_format(@do_date, '%Y-%m-%d 00:00:00') and
                bn.start_time < date_format(date_add(@do_date, interval 1 day), '%Y-%m-%d 00:00:00') and
               ...
               or
               (bn.start_time < date_format(@do_date, '%Y-%m-%d 00:00:00') and
                coalesce(bn.end_time, sysdate()) >= date_format(@do_date, '%Y-%m-%d 00:00:00') and
                coalesce(bn.end_time, sysdate()) < date_format(date_add(@do_date, interval 1 day), '%Y-%m-%d 00:00:00'))
               or
               (bn.start_time < date_format(@do_date, '%Y-%m-%d 00:00:00') and
                coalesce(bn.end_time, sysdate()) >=
                date_format(date_add(@do_date, interval 1 day), '%Y-%m-%d 00:00:00'))) or
       (bn.end_time is null and bn.start_time < date_format(date_add(@do_date, interval 1 day), '%Y-%m-%d 00:00:00'))
    );
	   

select * from qt_smartreport.qt_tmp_step1;	
...
(
    ...
    `the_hour_order_seconds`    decimal(10, 6)     DEFAULT NULL COMMENT '作业单在该小时内时长（秒）',
    ...
    `the_hour_cost_seconds`     decimal(10, 6)     DEFAULT NULL COMMENT '在该小时内时长（秒）',
...
(
    ...
    `the_hour_order_seconds_sum` decimal(10, 3)           DEFAULT NULL COMMENT '项目关联作业单时长（秒）',
    `add_notification_num`       int(100)           DEFAULT NULL COMMENT '新增告警次数',
    `notification_num`           int(100)           DEFAULT NULL COMMENT '告警次数',
...
insert into qt_smartreport.qt_notification_system_module_p1_index_stat(time_value, date_value, hour_value, time_type,index_value,index_value_fenzi,index_value_fenmu, value_type)
