select DATE_FORMAT(t.hour_start_time, '%Y-%m-%d %H:00:00')           as hour_value,
       coalesce(sum(t.uptime_state_duration) / sum(t.total_time), 0) as uptime_state_rate
      ...
      from (select t1.hour_start_time,
            ...
            from (select th.hour_start_time,
               ...
               left join (select tb.robot_code,
                          ...
                          where (tb.work_state in ('BUSY', 'CHARGING'))
                             or (tb.work_state = 'ERROR' and tb.job_sn is not null)
