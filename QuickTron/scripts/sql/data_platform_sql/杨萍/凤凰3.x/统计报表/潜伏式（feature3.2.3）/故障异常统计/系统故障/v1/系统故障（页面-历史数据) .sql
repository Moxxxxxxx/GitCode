...
#step7:建表（qt_hour_sys_error_time_detail_his）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_hour_sys_error_time_detail_his
(
    ...
    COLLATE = utf8_general_ci
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='各系统3~5级故障在小时内持续时长明细（T+1）';
	
	
	
#step8:删除相关数据（qt_hour_sys_error_time_detail_his）
DELETE
FROM qt_smartreport.qt_hour_sys_error_time_detail_his
where date_value=date_add(CURRENT_DATE(), interval -1 day);	


#step9:插入相关数据(qt_hour_sys_error_time_detail_his)
insert into qt_smartreport.qt_hour_sys_error_time_detail_his(date_value,hour_start_time, next_hour_start_time,alarm_service, the_hour_cost_seconds)
select date_add(current_date(), interval -1 day) as date_value,
       t1.hour_start_time,
       t1.next_hour_start_time,
       t2.alarm_service,
...
from (select th.day_hours                               as hour_start_time,
      ...
      from (SELECT DATE_FORMAT(DATE_SUB(DATE_FORMAT(date_add(CURRENT_DATE(), interval -1 day), '%Y-%m-%d 00:00:00'),
         ...
         inner join(select t3.alarm_service,
                           ...
                           t3.next_error_start_time,
                           case
                               when t3.start_time < date_format(date_add(current_date(), interval -1 day), '%Y-%m-%d 00:00:00.000000000') then date_format(date_add(current_date(), interval -1 day), '%Y-%m-%d 00:00:00.000000000')
                               else t3.start_time end                                              stat_start_time,
                           case
                               when COALESCE(t3.end_time, sysdate()) <=
                                    COALESCE(t3.next_error_start_time, t3.end_time, sysdate())
                               ...
                               when COALESCE(t3.end_time, sysdate()) >
                                    COALESCE(t3.next_error_start_time, t3.end_time, sysdate()) and
                                    COALESCE(t3.next_error_start_time, t3.end_time, sysdate()) < date_format(date_add(current_date(), interval -1 day), '%Y-%m-%d 00:00:00.000000000')
...
#step10:建表（qt_hour_all_sys_error_time_detail_his）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_hour_all_sys_error_time_detail_his
(
    `id`                    int(20)      NOT NULL AUTO_INCREMENT COMMENT '主键',
    `date_value`            date         NOT NULL COMMENT '日期',
    `hour_start_time`       datetime     NOT NULL COMMENT '小时开始时间',
    ...
    COLLATE = utf8_general_ci
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='全体系统3~5级故障在小时内持续时长明细（T+1）';
	
	
	
	
	
#step11:删除相关数据（qt_hour_all_sys_error_time_detail_his）
DELETE
...
#step12:插入相关数据(qt_hour_all_sys_error_time_detail_his)
insert into qt_smartreport.qt_hour_all_sys_error_time_detail_his(date_value,hour_start_time, next_hour_start_time,the_hour_cost_seconds)		 
select date_add(current_date(), interval -1 day) as date_value,
       t1.hour_start_time,
       t1.next_hour_start_time,
       sum(case
