...
select distinct hour_start_time as time_value,
                notification_id as index_value,
                '机器人故障id'       as index_name
...
where the_hour_cost_seconds is not null
union all
select t.order_create_time as time_value,
       t.order_no          as index_value,
       '搬运作业单id'           as index_name
from (select date_value,
             order_no,
             order_create_time,
             order_completed_time
      ...
      from qt_smartreport.qt_transport_order_detail_stat_realtime) t
         inner join (select td.order_no,
                     ...
                     from (select order_no, max(update_time) as update_time
                           from phoenix_rss.transport_order
                           group by order_no) td) ttd
                    on ttd.order_no = t.order_no and ttd.date_value = t.date_value