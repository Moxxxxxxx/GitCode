##step1:建表（qt_transport_order_next_step1_realtime）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_transport_order_next_step1_realtime
(
    id         bigint(20),
    order_no   varchar(32),
...
##step2:删除相关数据（qt_transport_order_next_step1_realtime）
DELETE FROM qt_smartreport.qt_transport_order_next_step1_realtime;



##step3:插入相关数据（qt_transport_order_next_step1_realtime）
insert into qt_smartreport.qt_transport_order_next_step1_realtime(id,order_no,current_id,next_id)
SELECT id,
       order_no,
...
##step4:建表（qt_transport_order_next_step2_realtime）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_transport_order_next_step2_realtime
(
    id         bigint(20),
    order_no varchar(32),
    next_id    varchar(32),
...
##step5:删除相关数据（qt_transport_order_next_step2_realtime）
DELETE FROM qt_smartreport.qt_transport_order_next_step2_realtime;



##step6:插入相关数据（qt_transport_order_next_step2_realtime）
insert into qt_smartreport.qt_transport_order_next_step2_realtime(id,order_no,next_id)
...
##step7:建表（qt_transport_order_link_detail_stat_realtime）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_transport_order_link_detail_stat_realtime
(
    ...
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='搬运作业单链路表(当天数据)';		
...
##step8:删除当天相关数据（qt_transport_order_link_detail_stat_realtime）
DELETE FROM qt_smartreport.qt_transport_order_link_detail_stat_realtime;  	




##step9:插入当天相关数据（qt_transport_order_link_detail_stat_realtime）
insert into qt_smartreport.qt_transport_order_link_detail_stat_realtime(date_value, upstream_order_no, order_no,
                                                               link_create_time, execute_state, order_state,
                                                               robot_code,first_classification,robot_type_name, next_link_create_time, next_execute_state,
...
from (select distinct upstream_order_no, order_no
	  ...
	  where update_time>=date_format(current_date(), '%Y-%m-%d 00:00:00.000000000') and update_time<=date_format(current_date(), '%Y-%m-%d 23:59:59.999999999')) t
...
##step10:建表（qt_transport_order_detail_stat_realtime）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_transport_order_detail_stat_realtime
(
    `id`                                int(20)      NOT NULL AUTO_INCREMENT COMMENT '主键',
    `date_value`                        date         NOT NULL COMMENT '日期',
    `upstream_order_no`                 varchar(100) NOT NULL COMMENT '上游作业单ID',
    ...
    COLLATE = utf8_general_ci
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='搬运作业单统计明细（当天数据）';	
...
##step11:删除当天相关数据（qt_transport_order_detail_stat_realtime）
DELETE FROM qt_smartreport.qt_transport_order_detail_stat_realtime;  	
...
--------------------------------------------------------------------------------------------------------------
##step12:插入当天相关数据（qt_transport_order_detail_stat_realtime）
insert into qt_smartreport.qt_transport_order_detail_stat_realtime(date_value, upstream_order_no, order_no, scene_type,start_point,
                                                                   start_area,
...
from phoenix_rss.transport_order t
         left join (select t.order_no,
                    ...
                    where t.update_time >=
                      ...
                      and t.update_time <=
                          date_format(current_date(), '%Y-%m-%d 23:59:59.999999999')
      ...
      from (select t1.order_no,
            ...
            from (select t.order_no, tk.id, tk.create_time
                  ...
                  where t.update_time >=
                        date_format(current_date(), '%Y-%m-%d 00:00:00.000000000')
                    and t.update_time <=
                        date_format(current_date(), '%Y-%m-%d 23:59:59.999999999')) t1
                     left join
                 (select t.order_no, tk.id, tk.create_time
                  ...
                  where t.update_time >=
                        date_format(current_date(), '%Y-%m-%d 00:00:00.000000000')
                    and t.update_time <=
                        date_format(current_date(), '%Y-%m-%d 23:59:59.999999999')) t2
     ...
     (select t.order_no,
      ...
      group by t.order_no) t5 on t5.order_no = t.order_no
         left join (select t.order_no,
                    ...
                    where t.update_time >= date_format(current_date(), '%Y-%m-%d 00:00:00.000000000')
                      and t.update_time <= date_format(current_date(), '%Y-%m-%d 23:59:59.999999999')
...
##step13:建表（qt_transport_upstream_order_detail_stat_realtime）
CREATE TABLE IF NOT EXISTS qt_smartreport.qt_transport_upstream_order_detail_stat_realtime
(
    `id`                                int(20)      NOT NULL AUTO_INCREMENT COMMENT '主键',
    `date_value`                        date         NOT NULL COMMENT '日期',
    `upstream_order_no`                 varchar(100) NOT NULL COMMENT '上游作业单ID',
    ...
    COLLATE = utf8_general_ci
    AUTO_INCREMENT = 1
    ROW_FORMAT = DYNAMIC COMMENT ='上游作业单统计明细（当天数据）';	
	
	
--------------------------------------------------------------------------------------------------------------
	
##step14:删除当天相关数据（qt_transport_upstream_order_detail_stat_realtime）
DELETE FROM qt_smartreport.qt_transport_upstream_order_detail_stat_realtime;  		



--------------------------------------------------------------------------------------------------------------
##step15:插入当天相关数据（qt_transport_upstream_order_detail_stat_realtime）
insert into qt_smartreport.qt_transport_upstream_order_detail_stat_realtime(date_value, upstream_order_no, scene_type,
                                                                            stat_time,
...
from (select tu.upstream_order_no,
      ...
      from (select distinct upstream_order_no
            from phoenix_rss.transport_order
            where update_time >= date_format(current_date(), '%Y-%m-%d 00:00:00.000000000')
              and update_time <= date_format(current_date(), '%Y-%m-%d 23:59:59.999999999')) tu
