select max(create_sys_error_num)                                                           as new_breakdown_num
     , case
           when max(create_order_num) = 0 then concat(max(create_sys_error_num), '/', '0')
           when max(create_order_num) >= max(create_sys_error_num) then CONCAT('1 / ',
                                                                               round(max(create_order_num) / max(create_sys_error_num)))
           when max(create_order_num) < max(create_sys_error_num) then concat(
                   round(max(create_sys_error_num) / max(create_order_num)), '/',
                   '1') end                                                                as sys_breakdown_carry_order_rate
     , case
           when max(create_job_num) = 0 then concat(max(create_sys_error_num), '/', '0')
           when max(create_job_num) >= max(create_sys_error_num) then CONCAT('1 / ',
                                                                             round(max(create_job_num) / max(create_sys_error_num)))
           when max(create_job_num) < max(create_sys_error_num) then concat(
                   round(max(create_sys_error_num) / max(create_job_num)), '/',
                   '1') end                                                                as sys_breakdown_carry_task_rate
