      ...
      where th.hour_start_time <= sysdate()) t1
         inner join(select t3.alarm_service,
                    ...
                    from (select t1.error_id,
                                 ...
                                 t2.alarm_service as next_alarm_service,
                          ...
                          from (select error_id,
                                       ...
                                       start_time,
                                       end_time,
                                       CONCAT(alarm_service, '-', @rn := @rn + 1) current_id,
                                       CONCAT(alarm_service, '-', @rn + 1)        next_id
                                from (select alarm_service,
                                     ...
                                     (SELECT @rn := '') tmp
                                order by alarm_service, start_time asc) t1
                                   left join(select error_id,
                                                    alarm_service,
                                                    start_time,
                                                    end_time,
                                                    CONCAT(alarm_service, '-', @rm := @rm + 1) current_id,
                                                    CONCAT(alarm_service, '-', @rm + 1)        next_id
                                             from (select alarm_service,
                                                          id as error_id,
                                                          start_time,
                                                          end_time
                                                  ...
                                                  (SELECT @rm := '') tmp
                                             order by alarm_service, start_time asc) t2
