##########################################################################################
-- 搬运作业单链路
set @now_start_time = date_add(current_date(), interval -1 day);
set @next_start_time = date(sysdate());
...
from phoenix_rss.transport_order_carrier_cost tc
inner join phoenix_rss.transport_order_link tol on tol.order_no = tc.order_no 
where tc.update_time >=@now_start_time and tc.update_time < @next_start_time)t2  on t2.order_no = tm.order_no and t2.id = tm.next_id
left join phoenix_basic.basic_robot br on br.robot_code = tol.robot_code
left join phoenix_basic.basic_robot_type brt on brt.id = br.robot_type_id
where tc.update_time >=@now_start_time and tc.update_time < @next_start_time
order by tol.order_no ,tol.create_time asc
...
coalesce(tj.empty_move_distance, 0) as empty_move_distance,
case when t.move_time_consuming !=0 then coalesce(tj.empty_move_distance, 0)/t.move_time_consuming else null end as empty_move_speed,	   
coalesce(tj.empty_move_distance, 0) as empty_move_distance,
case when t.rack_move_time_consuming !=0 then COALESCE(tj.loading_move_distance,0)/t.rack_move_time_consuming else null end as loading_move_speed,
...
# step1:建表（qt_transport_order_link_detail_stat_his）
...
# step2:删除相关数据（qt_transport_order_link_detail_stat_his）
DELETE
...
# step3:插入当天相关数据（qt_transport_order_link_detail_stat_his）
...
from phoenix_rss.transport_order_carrier_cost tc
inner join phoenix_rss.transport_order_link tol on tol.order_no = tc.order_no 
where tc.update_time >=date_add(current_date(), interval -1 day) and tc.update_time < date(sysdate()))t2  on t2.order_no = tm.order_no and t2.id = tm.next_id
left join phoenix_basic.basic_robot br on br.robot_code = tol.robot_code
left join phoenix_basic.basic_robot_type brt on brt.id = br.robot_type_id
where tc.update_time >=date_add(current_date(), interval -1 day) and tc.update_time < date(sysdate())
order by tol.order_no ,tol.create_time asc
...
# step4:建表（qt_transport_order_detail_stat_his）
...
# step5:删除当天相关数据（qt_transport_order_detail_stat_his）
DELETE
...
#step6:插入当天相关数据（qt_transport_order_detail_stat_his）
...
# step7:建表（qt_transport_upstream_order_detail_stat_his）
...
# step8:删除相关数据（qt_transport_upstream_order_detail_stat_his）
DELETE
...
# step9:插入相关数据（qt_transport_upstream_order_detail_stat_his）
