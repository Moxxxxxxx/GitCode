select 
th.hour_start_time as x,
ROUND(coalesce(sum(t.uptime_state_duration) / sum(t.uptime_state_rate_fenmu), 0)*100,2) as y
from 
(select 
date_format(concat(current_date(), ' ', hour_start_time),'%Y-%m-%d %H:00:00') as hour_start_time,
date_format(case when hour_start_time = '23:00:00'then concat(date_add(current_date(), interval 1 day),' ', next_hour_start_time) else concat(current_date(), ' ', next_hour_start_time) end,'%Y-%m-%d %H:00:00') as next_hour_start_time
from qt_smartreport.qt_dim_hour)th
left join
(select 
hour_value,
COALESCE(sum(t.uptime_state_duration),0) as uptime_state_duration,
COALESCE(sum(t.uptime_state_rate_fenmu),0) as uptime_state_rate_fenmu
from 
-- 当天当前小时之前的数据
(select 
...
group by hour_value)tu on th.hour_start_time=tu.hour_value
group by 1