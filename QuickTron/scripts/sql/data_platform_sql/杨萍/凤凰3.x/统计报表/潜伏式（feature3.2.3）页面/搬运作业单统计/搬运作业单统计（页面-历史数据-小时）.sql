...
insert into qt_smartreport.qt_hour_transport_order_link_detail_his(date_value,hour_start_time,next_hour_start_time, upstream_order_no, order_no,link_create_time, execute_state, order_state,robot_code, first_classification,robot_type_code,robot_type_name,next_link_create_time,next_execute_state,next_order_state,to_next_link_time_consuming)
select 
...
from phoenix_rss.transport_order_carrier_cost tc
inner join phoenix_rss.transport_order_link tol on tol.order_no = tc.order_no 
where tc.update_time >=date_format(DATE_ADD(sysdate(), INTERVAL -1 HOUR), '%Y-%m-%d %H:00:00') and tc.update_time < date_format(sysdate(), '%Y-%m-%d %H:00:00'))t2  on t2.order_no = tm.order_no and t2.id = tm.next_id
left join phoenix_basic.basic_robot br on br.robot_code = tol.robot_code
left join phoenix_basic.basic_robot_type brt on brt.id = br.robot_type_id
where tc.update_time >=date_format(DATE_ADD(sysdate(), INTERVAL -1 HOUR), '%Y-%m-%d %H:00:00') and tc.update_time < date_format(sysdate(), '%Y-%m-%d %H:00:00')
order by tol.order_no ,tol.create_time asc
