...
(
  ...
  (
    ...
    WHERE c.is_nonetwork = 1 AND c.project_product_type_code IN (1,2) and c.project_version like '2.%'
  )c
  LEFT JOIN
  (
    SELECT *
  ...
  (
    SELECT *
    FROM ${dim_dbname}.dim_collection_project_record_ful c
    WHERE c.is_nonetwork = 1 AND (c.project_product_type_code IN (3,4) OR c.project_product_type IN ('标准搬运')) and c.project_version like '2.%'
  )c
  LEFT JOIN
  (
    SELECT *
  ...
  (
    SELECT *
    FROM ${dim_dbname}.dim_collection_project_record_ful c
    WHERE c.is_nonetwork = 1 and c.project_version like '2.%' AND (c.project_product_type_code IN (3) OR c.project_product_type IN ('Quickpick','料箱搬运QP','QP'))
  )c
  LEFT JOIN
  (
    SELECT *
