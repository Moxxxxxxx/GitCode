--研发看板成员使用统计 ads_superset_rd_used_count

INSERT overwrite table ${ads_dbname}.ads_superset_rd_used_count
SELECT '' as id,
...
(
  ...
  (
    ...
    (
      ...
      WHERE find_in_set('7',aa.role_id)>0 or find_in_set('88',aa.role_id)>0
    ) auu
    LEFT JOIN 
    (
  ...
  (
    ...
    WHERE d.dashboard_name LIKE '%【大研发体系%'
  ) dd
