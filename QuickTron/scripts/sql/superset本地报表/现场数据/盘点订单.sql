...
SELECT DATE_FORMAT(cc.last_updated_date,'%Y-%m-%d %H:00:00') times,
	   COUNT(DISTINCT cc.id)order_num,
     0 'order_linenum',
     0 'sku_num',
     0 'into_station_times',
     0 'nodone_order_linenum'
FROM evo_wes_cyclecount.cycle_count cc
WHERE cc.state = 'DONE' AND cc.last_updated_date >= @begin_time and cc.last_updated_date < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num  MINUTE) -- AND cc.project_code = 'A51118'
...
WHERE ccd.last_updated_date >= @begin_time and ccd.last_updated_date < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num  MINUTE) -- AND ccd.project_code = 'A51118'
...
SELECT DATE_FORMAT(se.entry_time,'%Y-%m-%d %H:00:00') times,
	...
	WHERE se.biz_type LIKE '%CYCLE%' and se.entry_time >= @begin_time AND se.entry_time < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num MINUTE) -- AND se.project_code = 'A51118' 
...
SELECT 
     ...
     0 'order_linenum',
	   0 'sku_num',
     0 'into_station_times',
     count(distinct case when pwd.created_date <= theDayEndofhour then pwd.work_id end) - count(distinct case when pwd.updated_date <= theDayEndofhour then pwd.work_id end) as nodone_order_linenum -- T-1所处时间段累计未完成的订单行
FROM (
...
join evo_wcs_g2p.countcheck_work_detail pwd
left join
(
 SELECT *
 FROM 
 (
  SELECT pwd.project_code,pwd.work_id,pwd.updated_date
  FROM evo_wcs_g2p.countcheck_work pw 
  left join evo_wcs_g2p.countcheck_work_detail pwd 
  ...
  FROM evo_wcs_g2p.countcheck_work_detail pwd 
  left join evo_wcs_g2p.countcheck_job pj
 ...
 group by t.project_code,t.work_id
)t1
on t1.project_code = pwd.project_code and t1.work_id = pwd.work_id
where 1 = 1  -- AND ro.project_code = 'C35052' 
