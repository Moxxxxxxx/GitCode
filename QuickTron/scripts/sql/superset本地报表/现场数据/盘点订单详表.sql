...
WHERE ccd.last_updated_date >= @begin_time and ccd.last_updated_date < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num MINUTE)  AND ccd.station_code != ' ' AND ccd.station_code is not null -- AND ccd.project_code = 'A51118' 
...
SELECT DATE_FORMAT(pwd.updated_date,'%Y-%m-%d %H:00:00') times,
       ...
       0 'station_busy',
       0 'time'
    FROM evo_wcs_g2p.countcheck_work_detail pwd
    LEFT JOIN evo_wcs_g2p.countcheck_job cj
    ON pwd.id = cj.detail_id
	WHERE pwd.state= 'DONE'  AND pwd.updated_date >= @begin_time and pwd.updated_date < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num MINUTE) -- AND pwd.project_code = 'A51118' AND cj.project_code = 'A51118'
...
SELECT DATE_FORMAT(se.entry_time,'%Y-%m-%d %H:00:00') times,
	...
	WHERE idempotent_id LIKE '%G2PCountCheck%' and entry_time >= @begin_time AND entry_time < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num MINUTE) -- AND se.project_code = 'A51118'
...
SELECT 
   ...
   SUM(		
    ...
    WHERE 
       ...
       AND seq.idempotent_id LIKE '%G2PCountCheck%' -- AND seq.project_code = 'A51118' 
        ) tmp1
...
SELECT 
   ...
   0 'station_used',
    ...
    WHERE 
       ...
       AND sl.biz_type LIKE '%CYCLECOUNT%' -- AND sl.project_code = 'A51118' 
        ) tt1
...
WHERE sc.state = 'PULLED' AND sc.last_updated_date BETWEEN @begin_time AND DATE_ADD(@begin_time,INTERVAL 1 DAY) -- AND sc.project_code = 'A51118' AND t.project_code = 'A51118' AND cj.project_code = 'A51118' 
)tmp
ON jsc.job_id = tmp.job_id
WHERE jsc.updated_date >= @begin_time AND jsc.updated_date < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num MINUTE) AND jsc.state = 'DONE' -- AND jsc.project_code = 'A51118'  AND cj.project_code = 'A51118' 
GROUP BY ceil(TIMESTAMPDIFF(SECOND, @begin_time,jsc.updated_date)/@interval_time/60),cj.station_code
) tmp2 
