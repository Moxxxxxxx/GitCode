...
WHERE rod.quantity = rod.fulfill_quantity AND rod.last_updated_date >= @begin_time and rod.last_updated_date < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num MINUTE) AND w.replenish_mode LIKE '%GUIDED%' AND rod.project_code = 'A51118' AND w.project_code = 'A51118' 
...
SELECT DATE_FORMAT(se.entry_time,'%Y-%m-%d %H:00:00') times,
	...
	WHERE se.biz_type LIKE '%GUIDED%' and se.entry_time >= @begin_time AND se.entry_time < DATE_ADD(@begin_time,INTERVAL @interval_time*@line_num MINUTE) AND se.project_code = 'A51118'
...
SELECT 
   ...
   SUM(		
    ...
    WHERE 
       ...
       AND seq.biz_type LIKE '%GUIDED%' AND seq.project_code = 'A51118' 
        ) tmp1
...
SELECT 
   ...
   0 'station_used',
    ...
    WHERE 
       ...
       AND sl.biz_type LIKE '%GUIDED%' AND sl.project_code = 'A51118' 
        ) tt1
